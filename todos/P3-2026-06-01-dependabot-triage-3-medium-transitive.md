...
Triaged after merging PRs #317/#318. The GitHub push surfaced "3 moderate vulnerabilities on the default branch." Foundational reason none are urgent: all three are **build/dev/test-only** — none ships in the RN bundle or the bundled Express server runtime. (Original filing rationale was "no production deployment yet"; that premise is now partly stale — the **backend is live on Railway** (api.ocrecipes.com, since 2026-06-08), but the RN/web clients are not yet shipped. Re-checked 2026-06-22: none of the 3 are in the bundled Express runtime either, so the reachability conclusion holds; the pre-launch gate is now _partially_ active for the server artifact — see the 2026-06-22 Update.)
...
- [x] **Done (2026-06-22):** alert backlog fully cleared — **0 open** (17 fixed / 10 dismissed). #1 (esbuild) is now dismissed `not_used` (no longer "open"); that's a _reachability_ dismissal, **not** a fix — the on-disk `esbuild@0.18.20` mis-resolution still persists (see esbuild AC above, still blocked on upstream). Details in the 2026-06-22 Update.
...
### 2026-06-20 (expanded alert set re-triaged — 8 high, count now 19; folds in `P3-2026-06-20-dependabot-retriage-expanded-alerts`)

The default-branch count grew (3 moderate → live **19 open: 8 high, 6 moderate, 5 low** via `gh api .../dependabot/alerts?state=open`). The separate re-triage todo (`P3-2026-06-20-dependabot-retriage-expanded-alerts.md`) is **folded into this canonical home and archived**. The 8 **high** were triaged against the same reachability framework (production-reachable in the RN bundle / Express runtime AND attacker-reachable). **None qualify on both axes — nothing escalates out of P3.** Per-package (paths from `npm ls <pkg> --all`):

- **`undici` — 4 alerts (#24, #23 WS-client DoS `GHSA-vxpw-j846-p89q`; #22 SOCKS5 cross-origin `GHSA-hm92-r4w5-c3mj`; #19 SOCKS5 TLS-bypass `GHSA-vmh5-mc38-953g`). Defer-to-upstream.** Runtime package (`cheerio@1.2.0` URL-import fetch, `expo-server-sdk@6.1.0` push; also `jsdom` test, `@expo/cli` tooling), so the _package_ is partly production-reachable — **but every flagged CVE is in undici's WebSocket client or SOCKS5 `ProxyAgent`, neither of which this code uses** (cheerio/expo-server-sdk use plain HTTP fetch; no SOCKS5 proxy configured). Not attacker-reachable. Patched in 7.28.0 / 6.27.0; we have `undici@7.25.0` (deduped across cheerio/expo-server-sdk/jsdom) + `6.25.0` under `@expo/cli`. The next Expo SDK bump pulls it forward. A scoped `overrides: { undici: "^7.28.0" }` would lift the three 7.x instances but is not warranted (blunt-override guardrail; not reachable).
- **`form-data` — #15 (CRLF injection via unescaped multipart field names, `GHSA-hmw2-7cc7-3qxx`). Defer / optional patch.** Vulnerable `<4.0.6`; we have `4.0.5`. Paths are **test-only** (`supertest@7.2.2 → superagent`), **type-only** (`@types/supertest`/`@types/node-fetch` — `.d.ts` stubs, no runtime exec), and one nominal runtime path (`@apple/app-store-server-library@2.0.0 → @types/node-fetch`) that never constructs multipart with attacker-controlled field names (Apple's API is JWS/JSON). Not attacker-reachable. The 4.0.5→4.0.6 patch is low-risk if we want hygiene, but not urgent — fold into the pre-launch gate.
- **`ws` — 2 alerts (#12 7.x `<7.5.11`; #11 6.x `<6.2.4` — memory-exhaustion DoS `GHSA-96hv-2xvq-fx4p`). Defer-to-upstream.** Every _vulnerable_ instance is dev tooling: `metro` (0.83/0.84 → ws 7.5.10/7.5.11), `@react-native/dev-middleware` + RN dev packager (ws 6.2.3), `react-devtools-core` (ws 7.5.10). The only app-runtime consumer, `openai@6.39.0 → ws@8.21.0`, is **outside** the vulnerable range. Not production-reachable (these are localhost dev WebSocket servers). RN/Expo/metro bumps clear it.
- **`vite` — #9 (`server.fs.deny` bypass on Windows alt paths, `GHSA-fx2h-pf6j-xcff`). Tolerable_risk / defer.** Dev scope; `vitest@4.1.7 → vite@8.0.14` only. We never run a `vite` dev server (vitest uses it as a transform lib), and the bypass needs a running dev server + Windows. Not reachable. (Same `vite@8.0.14` is the esbuild-mis-resolution subject above — see the esbuild item.)

**Net:** the original reachability conclusion holds for the larger set — all 8 high are dev/build/test tooling or use unreachable vulnerable surfaces. No code change today. The 6 moderate / 5 low were not individually triaged (the high tier was the material jump); they ride the standing watchpoint. **Pre-launch gate still applies:** re-triage the full set against the _shipped_ artifact before the first production build (the `form-data` patch + an `undici` scoped override are the cheap hygiene items to revisit then). Live alert IDs drift as PRs merge (e.g. PR #404 `multer` bump already shifted the count from the 21/9-high seen at filing to 19/8-high here) — re-pull with `gh api` on revisit rather than trusting these numbers.

### 2026-06-22 (`/todo` re-verification — alert backlog now fully cleared; esbuild hygiene item still blocked)

Re-pulled live state (`gh api .../dependabot/alerts`, `npm ls`, `npm view`). **All alerts are now closed: 0 open** (down from 19 on 2026-06-20) — full history **17 fixed / 10 dismissed**. Verified twice (`--jq length` = 0). Auth/repo confirmed `Xertox1234/OCRecipes`.

- **esbuild #1 (`GHSA-67mh-4wv8-2f99`) is now dismissed `not_used`** — it was tracked here as "left **open**, the one actionable lockfile item." The dismissal reflects _reachability_ (vitest uses vite as a transform lib and never runs `esbuild serve`), **not a fix.** The underlying lockfile mis-resolution is **unchanged**: `npm ls` still reports `esbuild@0.18.20 invalid: "^0.27.0 || ^0.28.0"` under `vitest → vite@8.0.14`, hoisted via the deprecated `drizzle-kit@0.31.10 → @esbuild-kit/esm-loader@2.6.5 → @esbuild-kit/core-utils@3.3.2` chain (ELSPROBLEMS persists). **esbuild trigger still UNFIRED:** `drizzle-kit` latest = `0.31.10` (1.0 only at `1.0.0-rc.4`, pre-release), `drizzle-orm` latest = `0.45.2` (1.0 still pre-release). The dual pre-release-major adoption documented in the 2026-06-02 eval is still required and still a no-go.
- **Expanded 2026-06-20 set resolved:** undici / form-data / multer / postcss / qs / shell-quote → **fixed**; ws / vite / tar / js-yaml → **dismissed** (`not_used` / `tolerable_risk`). Likely PR #421 (transitive-`overrides` remediation) plus manual UI dismissals — that's the observed end state; per-alert causation not individually re-verified.
- **uuid #4 + brace-expansion #2** remain dismissed (`tolerable_risk`); on-disk unchanged — `uuid@3.4.0` (@expo/ngrok) / `uuid@7.0.3` (xcode), and `brace-expansion@1.1.12` under `eslint@9.39.1 → minimatch@3.1.5`. Their upstream upgrade paths _opened_ but are not adopted: **Expo SDK 56** and **eslint 10** are now published (were 54 / "not adopted" on 2026-06-20). Adopting either is separate large work and gated by the `dependabot.yml` `expo >=55` / version ignores.
- **Pre-launch gate now partially active:** the backend is live on Railway (api.ocrecipes.com, since 2026-06-08), RN/web clients not yet shipped. Re-checked the 3 against the shipped server artifact — none are in the bundled Express runtime (uuid = ngrok/xcode CLI tooling, brace-expansion = eslint, esbuild = build tool, not emitted into `server_dist/`), so the reachability conclusion holds. Full pre-launch re-triage still owed before the _client_ ships.
- **Status:** remains `blocked` (esbuild item genuinely blocked on upstream; all alerts closed, nothing actionable today). This supersedes the 2026-06-02 "stays `backlog`" note. Kept open as the standing watchpoint for the esbuild lockfile hygiene item + the pre-launch gate. No code change made — processed as a watchpoint re-verification, not dispatched to an executor.
