...
- [ ] MealPlanHome shows an inline error state with retry on `isLoadingError`, and keeps stale data on `isRefetchError`
- [ ] FeaturedRecipeDetail distinguishes a genuine 404 ("not found") from other errors (generic copy + retry)
