...
- [ ] **Emailed codes or links expire within 24 hours at most** (SP 800-63B-4's limit for recovery codes sent by email); the 15–60 minutes above is the target.
- [ ] **If a typed code is used, cap guesses.** The consume endpoint takes email + code, compares a stored hash in constant time, and allows at most about 5 failed attempts per issued code; after that the code is burned and the user must request a new one. Rate-limit consumption per account as well as per IP (per-IP alone falls to a distributed guesser). Minimum strength: 8+ random alphanumeric characters, or 6 digits only with the attempt cap enforced.
...
- [ ] **An email change invalidates outstanding reset tokens at the moment the address actually changes**, not when `change-email` stages it. With the verification gate on, `change-email` only calls `stagePendingEmail`; the address changes in `applyEmailVerification` (`server/storage/users.ts`, which swaps `pendingEmail` into `email`). With the gate off, it changes in `updateUserEmail`. Invalidate at both commit points. A password-hash fingerprint alone doesn't change on an email change, so the fingerprint must also cover the current verified email, or use a stored nonce with an explicit delete.
- [ ] Decide whether a completed reset marks the email as verified.
- [ ] A completed reset sends the user back to the login screen rather than logging them in (safer default), unless deliberately decided otherwise.
...
- [ ] Server and client tests for: happy path, expired token, reused token, unknown email (same response), a `pendingEmail` address (no reset sent), rate limit, sessions revoked immediately (cache invalidated), code brute force (burned after the attempt cap), and an email change committed after a reset was requested (the old reset token is rejected).
...
- The email-verification flow is the closest existing pattern (`server/lib/verification-token.ts`: its own audience, 24h TTL). A reset token, if it's a JWT, must be **distinguishable from an access token**. `requireAuth` accepts any token signed with the shared `JWT_SECRET` that has issuer `ocrecipes-api`, audience `ocrecipes-client`, and passes `isAccessTokenPayload`. So a reset JWT must use its own audience (e.g. `ocrecipes-password-reset`) and a `purpose` claim, pin `algorithms: ["HS256"]`, not reuse the access token's claim names (a separately named claim like `rv` is fine — see below), and use a short TTL. Otherwise it could double as a 7-day session. A stateless JWT is only single-use with a stored nonce or a fingerprint; pick one deliberately. A JWT payload is readable (base64), so a fingerprint must be an **HMAC keyed with a server secret** over the current password hash and verified email, never a slice of the bcrypt string (that would leak part of the hash in the link). A separate claim such as `rv`, compared against the user's `tokenVersion`, is fine: the separate audience, not the claim's absence, is what stops it working as an access token.
