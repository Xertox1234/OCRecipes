...
- Follow-up (PR #633): `getShutterClearanceStyle` no longer special-cases `session_complete` — it now gets the same insets-derived clearance as every other variant, which also eliminated the jump-cut noted above as an accepted tradeoff. `ZoomLabel.tsx`'s duplicated static `92` was also consolidated into a new shared `client/camera/components/shutter-layout.ts` (`getShutterTopInset`), closing the desync risk flagged in the bullet above.
- **User confirmed on-device**: the shutter/ProductChip overlap fix (round 1 + round 2) visually verified on a physical device. The environment-constraint caveat above (visual verification deferred, no simulator camera/dev-client) is now resolved — the acceptance criteria's "on any tested device size" requirement is satisfied.
