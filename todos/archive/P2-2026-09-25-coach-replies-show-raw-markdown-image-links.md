...
- Confirmation review: the empty-bubble guard (skip a bubble whose content strips to nothing) caused a regression. During streaming, a reply that is momentarily only a completed image tag rendered no bubble AND no "thinking"/status fallback, because the three streaming callers (StreamingBubble, ChatScreen's CoachStreamingFooter, CoachOverlayContent) gate their fallback on raw text. The guard was only a suggestion and the prompt now forbids bare images, so it is removed rather than threading a new predicate through three callers. Prompt wording corrected: search_recipes returns prep time, just not calories or protein.
