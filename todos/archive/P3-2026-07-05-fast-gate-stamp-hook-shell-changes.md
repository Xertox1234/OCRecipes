...
- [x] When `--fast` sees changed files under `.claude/hooks/**` or `.husky/**`, it runs
      `bash scripts/run-hook-tests.sh` before writing the stamp (fail → no stamp, non-zero exit).
- [x] The changed-hook detection uses the committed push range (same `BASE..HEAD` the existing
      `--fast` scope uses), not the staged/working set.
- [x] A hermetic `.claude/hooks/test-*.sh` case proves: changed hook file + failing hook test →
...
- [x] Decide and document whether an all-docs change (no TS, no hooks) should still stamp — current
      behavior stamps it; that is likely fine (nothing to verify) but should be a conscious choice.
- [x] `bash scripts/run-hook-tests.sh` stays green; the two consolidation tests
...
- Implemented. Added a second `HOOK_CHANGED` git-diff probe (same `BASE..HEAD` range as the
  existing `*.ts`/`*.tsx` `CHANGED` probe) over `.claude/hooks/` and `.husky/`, gating a
  `bash scripts/run-hook-tests.sh` call before the stamp write; a hook-test failure `exit 1`s
  before the stamp line is reached. Documented the AC #4 decision inline: an all-docs push
  (no TS, no hooks) still stamps — nothing to verify, nothing withheld. Added three hermetic
  cases to `.claude/hooks/test-preflight-fast-stamp.sh` (hook changed + pass → stamp, hook
  changed + fail → no stamp, hook file DELETED + fail → no stamp), run without
  `PREFLIGHT_DRY_RUN` since DRY_RUN would only echo the invocation. Code review (`code-reviewer`)
  caught that the probe's `--diff-filter=ACMR` (mirroring the TS `CHANGED` array) silently
  excluded pure deletions/rename-outs of hook files — fixed to `--diff-filter=ACDMR` (the probe
  is a pure boolean gate, never passed to eslint/vitest, so it has none of `ACMR`'s constraints)
  and locked in with the third hermetic case above. `bash scripts/run-hook-tests.sh` stays green
  (16/16); full verification (`test:run`, `check:types`, `lint`) all pass.

- Round-2 review (`code-reviewer`, scoped to the round-1 fix) found the `--diff-filter=ACDMR`
  fix still missed `T` (typechange, e.g. a hook file swapped for a symlink) — extended to
  `ACDMRT` — plus a minor test-symmetry SUGGESTION (add a "hook deleted + tests PASS → stamp"
  case alongside the existing "deleted + FAIL → no stamp" case). Both fixed inline (one-line,
  same files, in scope). Final state: 7/7 in `test-preflight-fast-stamp.sh`, 16/16 hook
  self-tests, `test:run`/`check:types`/`lint` all green.
