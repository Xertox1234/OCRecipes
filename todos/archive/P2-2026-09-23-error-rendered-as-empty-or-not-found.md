...
- Repair of PR #1095 after independent review: the meal-plan items error now renders in the scroll content (date strip, top actions and pull-to-refresh stay live) instead of a full-screen early return; both error announcements are edge-triggered on their own error (the budget one deferred while the skeleton hides it, so an items Try Again no longer re-announces it); the test mock derives `isError` so a bare-`isError` gate regression fails; the items announcement gained coverage; and the meal-plan recipe query in `FeaturedRecipeDetailScreen.tsx` opts out of the global toast via `meta: { silentError: true }`.
