...
- Confirmation review: the same-copy guard's test couldn't fail (in this repo both paths are equal, so a mutant hard-coding `ok: true` passed). The comparison is now the pure `pluginPathsMatch()`, tested with two distinct synthetic paths; the real-repo pin stays as a separate test. The memoization doc's "26 of 61" figure is re-measured and dated (25 of 60, 2026-09-25). Merged current `main` again; the ratchet reports 80/439, 0 new.
