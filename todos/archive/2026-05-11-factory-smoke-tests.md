...
# Add smoke tests for all factories under server/__tests__/factories/
...
Add a single smoke-test file that calls every factory with no args and asserts the returned shape passes the schema-derived type (which TypeScript already enforces, but the test also verifies *runtime* shape).
