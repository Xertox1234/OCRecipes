...
- The offline replay of a queued food log (`client/lib/offline-queue-drain.ts`) refreshed scanned items, daily summary and frequent items but not the daily budget, so Home's calorie header stayed stale after reconnect. It now invalidates the bare `["/api/daily-budget"]` prefix too (test updated). Also corrected the solution doc: `undefined` and `null` hash identically in an array slot; `date ?? null` is for readability.
- Independent review swept every server path that writes a daily log and found six client paths still skipping the calorie header (meal-plan confirm, discard, beverage log, photo confirm, label confirm, cook-session log). Added `QUERY_KEYS.dailyBudget` and a tested `invalidateFoodLogQueries` helper in `client/lib/food-log-invalidation.ts`; the five log/discard paths call it and meal-plan confirm adds the daily-budget key. Solution doc gained a Prevention section.
