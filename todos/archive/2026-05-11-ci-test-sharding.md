...
- [x] Decide shard count (start with 2 or 3 — gains diminish above 4 for a suite this size with one DB)
- [x] Update `.github/workflows/ci.yml`: convert the `ci` job to a matrix with `shard: [1, 2, 3]`, add `--shard=${{ matrix.shard }}/${{ strategy.job-total }}` to the test step
- [x] Each shard needs its own Postgres service (already configured per-job)
- [x] DB schema push and `pg_trgm` extension must run on each shard
- [x] Lint + types + pattern scripts can still run as a single non-shard job (they're fast)
