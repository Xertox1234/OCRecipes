...
- Security review found a pre-existing, more exploitable bug in the same parse step: a malformed percent-escape (e.g. `ocrecipes://nutrition/%C0`) makes `@react-navigation/core`'s default `getStateFromPath` throw `URIError`, and useLinking's live-tap listener calls it outside its try (uncaught; likely a one-tap crash in release, not device-verified). With the user's approval, fixed in this PR: the override catches `URIError` only and ignores the link. Tests for four malformed shapes plus a well-formed control; doc updated; the cap comment now says the timings are V8-only.
