...
- [~] **Concurrent-worktree safety:** DEFERRED (closed — see Updates 2026-06-26 #2). The env-strip + per-test temp repos enforce isolation for the hook self-tests (the actual corruption vector, now fixed). The broader "each agent owns its worktree / lock git-mutating ops" is open-ended process/doc work, **not a live defect** — the real corruption vectors are closed and the residual is already covered by existing mitigations: `drift-detect.sh` (warns on external HEAD drift), the now-registered `core-bare-guard.sh` (auto-heals external `core.bare` flips), and worktree isolation via `superpowers:using-git-worktrees`. Not implementing a speculative cross-agent git-mutation lock for a solo workflow.
