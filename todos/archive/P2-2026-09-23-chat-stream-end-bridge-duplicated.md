...
- Independent review found a parity break: RecipeChatScreen's capture gate had moved from raw to stripped content. A stream that is only an opening ```json fence strips to "", so the gate skipped that tick and kept an earlier fragment, which could surface as a garbage bubble if the recipe never parsed. Restored the raw-content gate and added an `isPresent`option to`usePendingAssistantBridge`(defaults to truthiness), so a bubble shows only when the captured content or recipe is present, exactly as before. The hook now documents that an object`streamingValue`needs monotonic fields. Tests: a screen-level strip-to-empty case (red before the fix) and a hook-level`isPresent` case. The review also confirmed neither chat screen has a Retry control, so "Response interrupted. Try sending again." is the accurate copy.
