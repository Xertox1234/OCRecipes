...
- Correction to the repair note above: the two new live-signal controls covered only the initial-call catch sites. The confirmation review showed a bare `if (abortSignal)` at just the two mid-stream sites still passed every test. Both existing mid-stream controls now pass a live, never-aborted signal; mutating only those two sites now fails both. All four catch sites are covered.
