...
residue) produces zero stdout lines exactly like a genuinely empty one, so it would be
misread as noise and an `rm -rf` attempted against it (whether that attempt actually
removes it depends on the underlying `rm`'s own permission handling) — where the
pre-#669 bare `[ ! -e ]` guard left it untouched unconditionally. Make the noise check
readability-aware (skip, don't replace, what it cannot inspect).
...
- [x] The replace branch runs only when the directory was actually inspectable — e.g.
...
- [x] Regression test in `.claude/hooks/test-worktree-deps.sh`: a `chmod 000`
...
- [x] Existing 20-case suite stays green; cache-noise replacement and real-install
