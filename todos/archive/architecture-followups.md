...
- [ ] **L18** Split `server/routes/recipes.ts` (1,026 LOC, 4 concerns) into: - `server/routes/recipes.ts` — community CRUD only - `server/routes/recipe-search.ts` — `/api/recipes/search` (MiniSearch) - `server/routes/recipe-catalog.ts` — Spoonacular endpoints - `server/routes/recipe-import.ts` — URL-import endpoints
