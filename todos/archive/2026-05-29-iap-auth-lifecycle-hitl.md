...
- **CLOSED.** H3 merged via #273 (CI-green; reviewed by kimi + code-reviewer). C1 (#270) / C4 (#271) / L1 / H3 all complete. The `client-state` rule from H3 was codified to `docs/rules/client-state.md` (#274). Afterward, `main` branch protection was added (required `ci.yml` checks + `enforce_admins`) so CI can no longer be bypassed by auto-merge — verified to block a deliberately-red PR. Todo archived.
