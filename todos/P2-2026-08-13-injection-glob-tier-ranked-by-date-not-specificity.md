...
- [ ] At least one other domain measured, **chosen for deep glob competition** (see the bimodal
      note in Background), to confirm the harness numbers are not idiosyncratic.
- [ ] The four out-of-pool `.claude/hooks/test-*.sh` docs are reachable — preferred fix is
