...
- [ ] A single shared mechanism supplies the header inset for scrollable screens under transparent headers (e.g. a `ScreenScrollView`/container component, or a `useScreenOptions` change that removes the per-screen obligation)
- [ ] SettingsScreen and at least a representative sample of the 22 `useHeaderHeight` screens are migrated to it (full migration may be staged)
- [ ] Behavior is identical on iOS and Android (no iOS-only props as the sole mechanism)
- [ ] A `docs/rules/` entry (react-native or design-system domain) states the canonical mechanism so new screens don't hand-roll insets
