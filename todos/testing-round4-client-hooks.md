...
- [ ] Install and configure `@testing-library/react` (or react-native variant) for hook testing
- [ ] Configure Vitest to handle React/JSX in hook test files (may need jsdom environment)
- [ ] `client/hooks/__tests__/useAuth.test.ts` — test auth state transitions, token management, network error fallback
- [ ] `client/hooks/__tests__/useChat.test.ts` — test SSE streaming logic, message parsing
- [ ] At least 3 additional hook tests for hooks with optimistic update logic
- [ ] All new tests pass alongside existing 1,135 tests
- [ ] `useAuth` coverage ≥ 70%
- [ ] No React Native import failures (mock native modules as needed)
