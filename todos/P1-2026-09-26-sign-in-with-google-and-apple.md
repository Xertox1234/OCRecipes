...
- [ ] **Server verifies identity tokens, never trusts the client.** Verify Google and Apple ID tokens against each provider's published JWKS: signature, issuer, **audience = our client IDs**, expiry, and a **nonce** bound to this sign-in attempt (a hashed nonce for Apple). The nonce is single-use: stored server-side, or signed with a short TTL, and consumed on verification. Accept only the exact list of Google client IDs (iOS, Android, web), and pin Apple's `aud` to the bundle ID / Services ID. Then issue our own session JWT.
