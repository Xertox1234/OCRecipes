...
- [x] **DECIDED by the user (2026-09-26): both an authenticator app (TOTP) and passkeys** are supported second factors. A user can enroll either or both.
...
- [ ] Still open for the user: is MFA **mandatory** for all accounts, or opt-in? The 8-character minimum is Rev. 4-compliant only for accounts that actually have MFA; with opt-in MFA, password-only accounts stay below it. Record the decision.
