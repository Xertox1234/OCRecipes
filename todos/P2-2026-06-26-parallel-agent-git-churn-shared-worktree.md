...
- [x] **`core.bare` guard:** built as a PreToolUse(Bash) hook `core-bare-guard.sh` (auto-resets `core.bare=true`→`false` before any git command, WARN-only). Chosen over husky (which runs _after_ git already refuses) / SessionStart (misses mid-session flips). Source of the flip confirmed external: VS Code's Git integration (this session's env carries `GIT_ASKPASS=.../Visual Studio Code.app/...`). (commit `209120df`.) **Pending: one-line `settings.json` registration — gate-blocked self-modification, handed to the user.**
...
- [ ] **Concurrent-worktree safety:** PARTIAL — the env-strip + per-test temp repos enforce isolation for the hook self-tests (the actual corruption vector). Broader "each agent owns its worktree / lock git-mutating ops" remains a process/doc item; deferred.
