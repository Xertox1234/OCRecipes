...
- [ ] The mechanism is documented where the next contributor will hit it — a comment at the top
      of the affected test file naming why the timezone is pinned, and stating that **any**
      non-UTC zone works (so nobody "fixes" a valid negative-offset pin).
...
- [ ] Re-derive empirically whether `PlanSlotPickerSheet.test.tsx` and
