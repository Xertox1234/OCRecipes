...
| Grocery lists        | `plan/grocery-list.yaml`                             | **Partial**: Plan → Grocery Lists screen renders (FAB asserted) and the create-list picker opens and dismisses — all mandatory. Empty-vs-populated is seeded-data-dependent and stays optional, and generating a list is not exercised (it needs an existing meal plan) |
