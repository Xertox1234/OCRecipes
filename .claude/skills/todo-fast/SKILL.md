...
- **`BLOCKED`:** re-dispatch that ONE implementer once (same two-attempt cap `todo-executor.md`'s Failure Path uses). If still `BLOCKED`, the **whole todo** fails: revert every implementer's changes (`git -C "$WORKTREE" checkout -- .`), report `failed` (Step 11's failure format), and go directly to **Cleanup** — skip Phases 6–10's verify/review/commit/PR pipeline entirely (Phase 11's own report is not skipped; it is replaced inline by the failure report just cited). **This revert only restores modified tracked files — it does not remove files an implementer created.** `checkout -- .` resolves paths through the index, and an untracked path has no index entry (measured: a modified tracked file returns to base content, a newly-created file survives untouched, exit 0, no error, no output). That gap is safe here only because the very next step is Cleanup's `git worktree remove --force`, which discards the whole worktree — any surviving created file included — before anything downstream could commit or PR it. If a future change keeps the worktree alive after a `BLOCKED` failure (a retry, a salvage path, a debugging hold), this revert must first be replaced with the existence-split loop `todo-executor.md`'s Failure Path uses (`git cat-file -e "$BASE:$f"` → `git checkout -- "$f"` or `rm -f "$f"`) — see `docs/solutions/logic-errors/reset-mixed-revert-skips-files-the-commits-created-2026-09-20.md`. A todo with unmet Acceptance Criteria is never archived, regardless of how many other sub-tasks independently succeeded.
...
- **A todo with any `BLOCKED` sub-task after retry is discarded entirely** — the tracked-file revert plus Cleanup's worktree teardown together guarantee nothing survives to a commit or PR; no partial-credit half-merges.
