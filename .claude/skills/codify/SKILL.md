...
**`tags:` MUST contain at least one routed domain, or the doc never injects.** The hook builds each domain's candidate pool by grepping `^tags:.*<domain>` **before** `applies_to` is consulted, so a doc tagged only with descriptive words (`[parsing, validation, units]`) is in no pool at all and `applies_to` cannot rescue it. Resolve the domains for a path your doc covers with `npx tsx scripts/lib/path-domains.ts <path>` and include at least one. The 14: `accessibility, ai-prompting, api, architecture, client-state, database, design-system, harness, hooks, performance, react-native, security, testing, typescript` — plus two alternations, `harness` also accepting `tooling`/`pg-lab`/`worktree(s)`/`agents`, and `ai-prompting` accepting any `ai-*` tag. `scripts/check-solution-frontmatter.js` (lint-staged) fails the commit otherwise, so getting this right here avoids a blocked commit in Step 7. Some paths route **nowhere** (`shared/lib/**`) — the CLI prints nothing and exits 0 for those, which means pick a different `applies_to` target rather than that no tag exists. Since PR #799, `docs/solutions/**` and `docs/rules/**` route to `harness`, so a doc _about_ the knowledge base is reachable with a `harness` tag.
