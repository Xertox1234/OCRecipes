...
Each audit domain maps to reviewer agents in `.claude/agents/` that have deep knowledge of the project's patterns, conventions, and common pitfalls for that domain. Launch them as subagents during Phase 2 discovery. (Domain→reviewer routing is owned canonically by `.claude/skills/codify/SKILL.md` Step 2/Step 5 — this table applies it to audit scopes.)

| Audit Domain      | Primary Agent(s)                                              | What They Check                                                                                                                                                                                                                                                                                                                                         |
| ----------------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `security`        | `security-auditor` + `ai-reviewer`                            | IDOR, rate limiting, JWT, SSRF, prompt injection, AI safety                                                                                                                                                                                                                                                                                             |
| `performance`     | `mobile-reviewer` + `server-reviewer`                         | FlatList memoization, useCallback stability, streaming UI, Promise.all, N+1 queries, missing indexes                                                                                                                                                                                                                                                    |
| `data-integrity`  | `server-reviewer` + `ai-reviewer`                             | Soft deletes, polymorphic FK orphans, cache dedup, nutrition accuracy                                                                                                                                                                                                                                                                                   |
| `architecture`    | `server-reviewer`                                             | Service/storage layering, dependency direction, route module structure, SSE patterns, singleton init                                                                                                                                                                                                                                                    |
| `code-quality`    | `code-reviewer`                                               | Error handling, naming, type guards, Zod schemas, nav typing, test coverage gaps                                                                                                                                                                                                                                                                        |
| `camera`          | `mobile-reviewer`                                             | Permissions, scan debouncing, frame processors, lifecycle management                                                                                                                                                                                                                                                                                    |
| `accessibility`   | `mobile-reviewer`                                             | Modal focus trapping, VoiceOver/TalkBack announcements, touch targets, WCAG contrast, aria-invalid                                                                                                                                                                                                                                                      |
...
**For `full` scope:** Launch **one agent invocation per structural domain row** (the 7 rows above `reliability`; 7 total). Batch in two groups — first 4 domains, then 3 — to avoid overwhelming context. The "Primary Agent(s)" column shows which agent type to use for each invocation. **When a row lists multiple agents, use the first-listed agent as the `subagent_type` and explicitly name each remaining agent's focus in the prompt body** (e.g., for `security`: "Apply both the security-auditor lens (IDOR, JWT, rate limiting, SSRF) and the ai-reviewer lens (prompt injection, AI safety)"). `full` does **not** include `reliability` or `maintainability`.
...
| ------------------------- | ------- | --------------------------------------------------- |
| Server resilience         | 1-3     | `server-reviewer` + `security-auditor`              |
| Client reliability        | 4-7     | `mobile-reviewer` + `code-reviewer`                 |
| Cross-cutting correctness | 8-9     | `server-reviewer` + `ai-reviewer` + `code-reviewer` |
| Detection / observability | 10      | `server-reviewer` + `code-reviewer`                 |
...
**No new infrastructure:** reliability reuses the existing reviewer agents and the existing `docs/rules/` files. It does **not** add a dedicated reliability agent or a `docs/rules/reliability.md`.
...
**Standalone caveat:** `/audit maintainability` alone is _one_ perspective. The "two perspectives at discovery" benefit only shows up when this lens runs in parallel with structural domain reviewers (`pre-launch` and `code-quality`). Use standalone when you want a focused structural-simplification pass without the defect lens.
...
1. **Launch reviewer agents in parallel** using the mapping table above:
   ...
   - `code-quality` scope: launch `code-reviewer` (which carries the quality/TypeScript/testing lenses) **plus** the maintainability dispatch — 2 invocations in parallel, the second perspective being the structural-quality lens
...
1. **Launch doc-validation researcher agents in parallel (subagent type `general-purpose` — the dispatch prompt below is their full instruction set)** — one per audit domain that has at least one finding:
   - `full`: launch one researcher per structural domain with findings, batched the same way Phase 2 batches its reviewer agents (first 4 domains, then the rest)
...
1. **Select reviewers for the full diff** (per Review Policy): the relevant **domain reviewers** for the union of all touched domains across the fixes, **plus** the `code-reviewer` generalist — always, here, because Phase 6's distinctive value is the cross-file + structural-quality pass that only the generalist carries. Dispatch them **in parallel** over the list of all modified files from Phase 3, giving each the one-line fix descriptions (from the manifest) for context. **Working-tree safety:** in your own cwd capture `WORKTREE=$(git rev-parse --show-toplevel)` plus the audit branch and short HEAD, and have every reviewer (domain reviewers and `code-reviewer` alike) follow the **"Working-tree safety" procedure in `docs/AI_WORKFLOW.md` → Review Policy** — `git -C "$WORKTREE"` for all git commands, read files at `$WORKTREE/<path>`, and open with the tree check; not restated here.
   - **Domain reviewers**: use the Review-Policy dispatch prompt, scoped to the modified-file set, reviewing through each lens. Use the working-tree diff variant — `git -C "$WORKTREE" diff HEAD -- <modified files>`, **not** `main...HEAD`: audit fixes stay uncommitted until Phase 7, so at Phase 6 the branch HEAD equals its base and `main...HEAD` shows nothing — a false "No findings" that silently passes unreviewed code.
...
5. **Run the single authoritative full-suite gate** — `npm run test:run` (all pass), `npm run check:types` (zero errors), `npm run lint` (zero errors). Run it **even if the review produced no fixes** (Phase 5 no longer runs the suite); this is the audit's one post-fix full-suite confirmation.
6. **Append the CHANGELOG entry** drafted in Phase 5 to the main checkout's CHANGELOG (`"$MAIN_CHECKOUT/docs/audits/CHANGELOG.md"`) — now that the gate (step 5) is green, the append-only log records a confirmed-green audit. (The worktree's copy would vanish at Phase 9, so it must be written to the main checkout.)
...
After fixes are committed, extract reusable knowledge inline from the audit manifest and update reviewer agents with new checks.
...
3. **Audit-specific routing** (domains the codify table does not cover):
   ...
   | Finding Domain  | Update Target                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
   | --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
   | Reliability     | route by cluster to the agents in the "Reliability Scope" table (no dedicated reliability agent); rules go to the matching existing `docs/rules/{domain}.md` (e.g. Class 8 → `typescript.md`/`database.md`, Class 9 → `database.md`, Class 6 → `security.md`) — never a new `reliability.md`                                                                                                                                                                                                                                             |
