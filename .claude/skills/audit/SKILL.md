...
- **Supply-chain pins:** the npx-run tools (knip/jscpd/madge) are pinned to exact versions in the script's `NPX_PINS` constant — never loosen a pin to `latest`; bump versions deliberately in the script. All scanners run against the repo root resolved via git (never the ambient cwd). A top-level pin freezes only the named package, not its transitive dependency tree (no lockfile) — it raises the bar, it doesn't close the supply-chain gap.
- **gitleaks findings need rotation confirmation, not just row-copy:** gitleaks scans git _history_, so a finding's rule-id + file:line can point at an already-rotated secret. Copying the row into the manifest records where it lived — it does not confirm the credential is dead. Verify rotation status before closing a gitleaks-sourced finding.
