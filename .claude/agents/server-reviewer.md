...
- [ ] Append-only "value probe" / telemetry ledger tables (e.g. usage counters that gate a later keep/revert decision) must log a row for **every invocation that reaches the DB layer** — including a miss, or a reachable-but-empty/unpopulated source — not only hits. If only successes get logged, "never invoked / the pipeline is broken" becomes indistinguishable from "invoked and genuinely found nothing," which corrupts whatever later decision the ledger is meant to drive. Concrete instance: `scripts/pg-lab/codify-neardup.sh`'s `harness.codify_neardup_log` (a `top_score = NULL` row on a reachable-but-empty query result, vs. no row at all when the DB connection itself fails).
