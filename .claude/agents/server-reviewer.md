...
- **A response field the server computes from data it might not have must be `T | undefined` (key omitted), never a defaulted `T`.** In a downstream precedence chain (explicit flag → heuristic → give up), a confidently-wrong default **short-circuits the fallback that would have been right**, so shipping the field is worse than omitting it. Check the helper's empty-input behaviour, not just its name: `isBeverage: isBeverageCategory(tags ?? [])` looked guarded, but `isBeverageCategory([])` returns `false` and `extractOffUniversalData(null)` yields `categoriesTags: []` — so the `??` never fired and **every** USDA-only barcode shipped a confident `false`, forcing real drinks onto the food nutrient scale at half strictness. Gate on evidence at the call site (`tags && tags.length > 0 ? classify(tags) : undefined`) and leave the helper alone. Demand tests that use the encoding **production actually emits** (here `[]`, not an omitted key — the original test probed a shape the real path cannot produce), that assert absence with `not.toHaveProperty(...)` rather than `toBeUndefined()` (which also passes for a present `null`), and that keep one genuine-negative case proving the field still emits `false` — otherwise "gate the default" and "stop emitting the field" are indistinguishable. (Ref: `docs/solutions/logic-errors/absent-field-beats-defaulted-one-in-a-precedence-chain-2026-07-31.md`)
