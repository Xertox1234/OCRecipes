...
- **A response field the server computes from data it might not have must be `T | undefined` (key omitted), never a defaulted `T`.** In a downstream precedence chain (explicit flag → heuristic → give up), a confidently-wrong default **short-circuits the fallback that would have been right**, so shipping the field is worse than omitting it. Check the helper's empty-input behaviour, not just its name: `isBeverage: isBeverageCategory(tags ?? [])` looked guarded, but `isBeverageCategory([])` returns `false` and `extractOffUniversalData(null)` yields `categoriesTags: []` — so the `??` never fired and **every** USDA-only barcode shipped a confident `false`, forcing real drinks onto the food nutrient scale at half strictness. Gate on evidence at the call site (`tags && tags.length > 0 ? classify(tags) : undefined`) and leave the helper alone. Demand tests that use the encoding **production actually emits** (here `[]`, not an omitted key — the original test probed a shape the real path cannot produce), that assert absence with `not.toHaveProperty(...)` rather than `toBeUndefined()` — the two diverge only on a key that is PRESENT holding `undefined` (`toHaveProperty` catches it, `toBeUndefined()` passes); both correctly fail on a present `null`, and parsed JSON cannot hold an own `undefined`, so on a real `res.body` they are equivalent and the divergence bites on hand-built mocks/fixtures instead — and that keep one genuine-negative case proving the field still emits `false` — otherwise "gate the default" and "stop emitting the field" are indistinguishable. (Ref: `docs/solutions/logic-errors/absent-field-beats-defaulted-one-in-a-precedence-chain-2026-07-31.md`)
...
Before reporting an export as dead / orphaned / safe-to-delete, clear four checks — `findReferences` alone is necessary but **not** sufficient:
...
4. **A field on an API RESPONSE BODY is never "dead" on grep evidence alone — it needs deployed-bundle evidence.** A clean grep proves no reader in the current source tree; it says nothing about bundles already running on users' devices. This app ships JS over the air: EAS Updates apply on the **second** cold start, and there is an embedded-vs-OTA split, so the at-risk window is app-store/update-adoption time, not commits. Keeping a dead field costs bytes; removing one a live bundle reads yields `undefined` in that bundle and is **not reversible for that user**. Also classify each grep hit before trusting it — a match can be a derived-value coincidence (`NutritionDetailScreen` matches `nutriScore` but renders `partition.nutriScore`, a `ScanFlag` built from `orderedFlags`, not the raw scalar) or a test fixture. Flag the trim as a QUESTION with the trigger condition, not a deletion. **If the justification cites a removal date, verify it from git** (`git log --format='%h %ad %s' --date=short -S'<field>' -- client/ shared/`, then `git show <sha>` to confirm the commit removed rather than added it) — the window is computed from that one date, and a todo filename or the commit that ADDED the field are both easy wrong grabs. Precedent: `server/routes/nutrition.ts` `novaGroup`/`nutriScore` (PR #713 — kept, comment corrected; its first draft cited 2026-07-22, the date #694 ADDED the client fields, instead of 2026-07-24 when #708/`13bf5059` removed them). Exceptions: never-shipped fields, and licence/privacy-driven strips (ODbL `additivesTags`/`categoriesTags`), which outrank compatibility. See `docs/solutions/conventions/wire-field-removal-needs-deployed-bundle-evidence-2026-07-25.md`.
...
- [ ] Append-only "value probe" / telemetry ledger tables (e.g. usage counters that gate a later keep/revert decision) must log a row for **every invocation that reaches the DB layer** — including a miss, or a reachable-but-empty/unpopulated source — not only hits. If only successes get logged, "never invoked / the pipeline is broken" becomes indistinguishable from "invoked and genuinely found nothing," which corrupts whatever later decision the ledger is meant to drive. Concrete instance: `scripts/pg-lab/codify-neardup.sh`'s `harness.codify_neardup_log` (a `top_score = NULL` row on a reachable-but-empty query result, vs. no row at all on any DB/query-layer error — connection failure, missing table, or otherwise — the script can't and doesn't distinguish these by design). **A sentinel for the empty source is not a freshness signal** — it fires only at the one endpoint where the source has zero rows, so a _partially_ stale source (615 of 768 rows, frozen a month earlier) logs a perfectly normal score and the ledger reads as healthy. When the measured source is a manually-rebuilt derived projection, record its size/extent on each probe row, or refresh it where the source is written — otherwise the dated decision the probe exists to drive is made about a degraded feature, with nothing in the ledger saying so. See `docs/solutions/logic-errors/freshness-guard-as-emptiness-check-passes-when-partially-stale-2026-08-09.md`.
- [ ] **A column add in a schema `.sql` file must name the mechanical path by which it reaches every standing database** — a migration runner, an idempotent self-apply in the loud entry points (the pg-lab report-script idiom: `PGOPTIONS='-c client_min_messages=warning' psql -v ON_ERROR_STOP=1 -f schema/<file>.sql`), or an explicit one-time apply in the PR. Reject "the file is idempotent" as the answer — idempotent-but-never-applied is unapplied. This is CRITICAL when writers are fail-silent (`>/dev/null 2>&1 … || true`): an INSERT naming the new column against an old-schema DB silently drops **every** row, not just the new field. Also: the column add itself follows ALTER-only declaration (never restated in `CREATE TABLE` — the `eval-results.sql` convention), a separate `ALTER COLUMN … SET DEFAULT` statement (an inline DEFAULT on `ADD COLUMN IF NOT EXISTS` no-ops where the column already exists), and an idempotent NULL backfill so "absent" has ONE encoding. See `logic-errors/unapplied-schema-plus-fail-silent-writers-total-row-loss-2026-07-20`.
