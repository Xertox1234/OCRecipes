...
**`accessibilityViewIsModal` is iOS-only — in-screen overlays need a separate Android trap.** The prop is a **no-op on Android**. A React Native `<Modal>` already traps focus on both platforms (new window), so it needs nothing extra. But an _inline_ overlay — a conditionally-rendered sibling `View`, not an RN `<Modal>` (confirm cards, product chips, action panels, scan overlays) — leaves the controls behind it **reachable by TalkBack**. Hide the **behind-content** (the overlay's siblings, NOT the overlay itself) with `importantForAccessibility="no-hide-descendants"` (restore with `"auto"`); it is itself a no-op on iOS, so the iOS path is untouched. Apply it **per-element, not via a wrapper** (a wrapper re-scopes absolutely-positioned `zIndex` children and can flip paint order). When two overlays can stack, compute the per-surface values in one **tested pure function** — the surface that is itself the active, un-superseded overlay must stay reachable. This is the modal-trap case (rule 3): do **not** reach for `accessibilityElementsHidden` (that is the rule-17 visual-hide pattern; on iOS the overlay's `accessibilityViewIsModal` already hides its siblings, so it would be redundant and touch the iOS path). See `docs/solutions/conventions/in-screen-overlay-needs-android-focus-trap-2026-06-22.md`.
...
**Container live region re-reads the WHOLE subtree on ANY descendant change.** Flag any `accessibilityLiveRegion="polite"`/`"assertive"` on a **container** (not a leaf) that wraps a frequently-mutating child — a `Text↔ActivityIndicator` spinner swap, a control with `accessibilityState={{ busy }}`, or a live value. On Android, TalkBack re-speaks the container's _entire_ accessible text on the change, not just the change — confirmed on `ProductChip` (the busy swap re-read the full chip, `nodeLiveRegion=1`). Three traps when proposing a fix: (1) `accessibilityLiveRegion="none"` on the swapping **child** does NOT help — the container is the announcer; (2) removing/narrowing the container region often **silently mutes** other state transitions on Android, because that region is usually the _only_ Android announcer (iOS uses a gated `announceForAccessibility`); (3) replacing it with an imperative `announceForAccessibility` keyed on the render **discriminator** (`[variant]`/`[type]`/key) drops **same-discriminator content updates** — an async value that fills in while the discriminator stays the same (e.g. `BARCODE_LOCKED`→`PRODUCT_LOADED` loading the product name in place) goes silent on both platforms, where the live region used to speak it on Android. Replacement announces must be keyed on the changed **content**, not just the discriminator; add an edge-guarded content-keyed effect per in-place value. Map the live region's full blast radius across every state the component renders before changing it. **A variant-stepped manual/render sweep cannot catch trap (3)** — it never produces a same-discriminator content change; add an explicit placeholder→value-attached case. See `docs/solutions/conventions/android-container-live-region-reannounces-whole-subtree-2026-06-23.md` and `docs/solutions/logic-errors/imperative-announce-must-be-content-keyed-not-variant-keyed-2026-06-24.md`. Verify on-device/emulator per `docs/solutions/best-practices/verify-talkback-behavior-via-emulator-logcat-2026-06-23.md`.
