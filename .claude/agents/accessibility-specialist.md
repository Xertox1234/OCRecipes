...
**Container live region re-reads the WHOLE subtree on ANY descendant change.** Flag any `accessibilityLiveRegion="polite"`/`"assertive"` on a **container** (not a leaf) that wraps a frequently-mutating child — a `Text↔ActivityIndicator` spinner swap, a control with `accessibilityState={{ busy }}`, or a live value. On Android, TalkBack re-speaks the container's _entire_ accessible text on the change, not just the change — confirmed on `ProductChip` (the busy swap re-read the full chip, `nodeLiveRegion=1`). Two traps when proposing a fix: (1) `accessibilityLiveRegion="none"` on the swapping **child** does NOT help — the container is the announcer; (2) removing/narrowing the container region often **silently mutes** other state transitions on Android, because that region is usually the _only_ Android announcer (iOS uses a gated `announceForAccessibility`). Map the live region's full blast radius across every state the component renders before changing it. See `docs/solutions/conventions/android-container-live-region-reannounces-whole-subtree-2026-06-23.md`. Verify on-device/emulator per `docs/solutions/best-practices/verify-talkback-behavior-via-emulator-logcat-2026-06-23.md`.
