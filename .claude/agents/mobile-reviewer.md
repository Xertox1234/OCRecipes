...
- [ ] **A shared container any caller can pass taller content into uses `minHeight`, never `height`** — and `alignItems: "center"` on a fixed cross axis is a smell. RN does not clip by default, so an oversized child is not clipped or scrolled: `center` splits the excess and **draws it outside the border on both sides**. The visible symptom (text crossing the container's border, caret on top of a label) is far removed from the cause, and every short-content usage still looks correct — only the caller passing `minHeight`/multiline exposes it. Fix is `minHeight` + `alignItems: "stretch"` + `alignSelf: "center"` on icons. When replacing a child's `height: "100%"` with a flex rule, re-check the **tap target** separately (≥44pt): a percentage height needs a definite parent, so under `minHeight` it silently resolves to nothing while looking identical. Prefer `flexGrow: 1` (keeps intrinsic size so the child's `minHeight` still grows the parent). See `docs/solutions/logic-errors/fixed-height-flex-row-overflows-instead-of-growing-2026-07-27.md`
...
- [ ] **`navigation.reset()` silently no-ops when it must dismiss two or more stacked native modal presentations.** `reset` is a wholesale state REPLACEMENT; `react-native-screens` reconciles JS state against the real native view-controller stack, and when the native side doesn't tear both presentations down it reverts JS state to match — so the action is dropped with no throw, no warning, and an unchanged `getState()`. Flag `reset` used to reach a screen beneath 2+ `presentation: "modal"`/`"fullScreenModal"` boundaries; require a POP-family action (`popTo(name, params)`, `popToTop()`), which maps to the native dismissal path. `reset` stays fine where nothing is presented (e.g. a `safeGoBack` fallback that only runs when `canGoBack()` is false). Also flag the test shape that hides it: a mocked `useNavigation` can only prove WHICH action was dispatched, never that the platform honoured it — asserting `toHaveBeenCalledWith` on the broken call passes for the entire life of the bug. (Ref: `docs/solutions/logic-errors/navigation-reset-silently-noops-across-stacked-modals-2026-07-30.md`)
...
- [ ] **A `translateY` hand-derived from layout is wrong whenever a `scale` sits in the same transform list.** `transformOrigin: "left"` pins ONLY the horizontal axis — the vertical origin stays `center`, so the element shrinks toward its own middle and its visible top lands `(lineHeight - lineHeight * scale) / 2` BELOW the transformed box top. The error is small and constant, so it reads as "the design is slightly off" rather than a bug (precedent: the floating label in `TextInput.tsx` stopped short and overlapped the value text). Model the inset in a tested pure helper; don't compensate by inflating the translate, which fixes the position and leaves the gap wrong. The two-value `transformOrigin: "left top"` has weaker support than the one-value form — a silent fallback to center-origin reintroduces the bug, so model the inset regardless. Any component positioning against font metrics declares an explicit `lineHeight`. See `docs/solutions/logic-errors/transform-origin-left-still-scales-from-vertical-center-2026-07-27.md`
...
- [ ] **`BottomSheetModal.onChange` alone has a dead window during the opening animation.** `@gorhom/bottom-sheet`'s `onChange(index)` fires only once an animation COMPLETES (`animateToPositionCompleted`), not when it starts — a presented-state ref/flag derived only from `onChange`, for an imperatively-presented sheet (no React `isOpen` state), reads stale ("closed") for the ~200-300ms the sheet is visibly animating open. Any consumer of that state during the animation (hardware-back interception, backdrop press-through, focus trapping) misbehaves. Flag any imperative `BottomSheetModal` host deriving open/closed state from `onChange` without also wiring `onAnimate` (fires at animation START, before `onChange`) to flip the flag `true` as soon as `toIndex >= 0` — never flip it `false` from `onAnimate`, only from `onChange(-1)`, so a close animation biases toward "still open" rather than risking a false-closed read. See `docs/solutions/logic-errors/gorhom-onchange-fires-on-animation-complete-not-start-2026-07-07.md`, `client/hooks/useSheetBackHandler.ts`
...
- [ ] **Two imperative `announceForAccessibility` calls in the SAME render commit collide on iOS** — VoiceOver drops one (its `UIAccessibility.post(.announcement)` does not queue; the second interrupts the first before it is spoken), so on a safety surface the important announcement can be silently swallowed. Flag two separate effects that each announce on an undefined→defined transition when their trigger state can become defined together (e.g. product name + a safety-flag badge arriving in one `PRODUCT_LOADED` dispatch). Fix: on iOS fold co-arriving pieces into ONE combined utterance; keep the platform split (Android announces only the name imperatively — the badge's own `accessibilityLiveRegion` reads the flag, so a second imperative call would double it). A regression test MUST assert the announce **call count** (`toHaveBeenCalledTimes(1)`), not just that the combined string was announced — a string-only assertion passes even if the split path fired twice. See `docs/solutions/logic-errors/two-announceforaccessibility-same-commit-collide-ios-2026-07-21.md`
...
- [ ] **Downgrading an EXISTING `accessibilityLiveRegion="assertive"` to `"polite"` (or removing it) on an element that is the ONLY Android signal for safety-critical content (an allergen/danger flag badge with no fallback imperative announce on Android) is a WARNING unless the diff cites on-device/emulator TalkBack verification** (`docs/solutions/best-practices/verify-talkback-behavior-via-emulator-logcat-2026-06-23.md`). `"polite"` events can be deferred behind a concurrently-firing announcement (e.g. a product-name `announceForAccessibility` in the same commit) — unverified, that read as "probably fine" but the actual TalkBack behavior (queued-and-spoken vs. coalesced-and-lost) is not something to assume from the API docs alone on a safety surface
...
- [ ] **`accessible={true}` on a container swallows an interactive descendant, not just decorative ones.** `accessible={true}` collapses the ENTIRE subtree into one VoiceOver/TalkBack focus stop — flag it whenever the container has an interactive child (a Pressable action button, not just an icon/badge), even if a companion `announceForAccessibility` call makes it _sound_ like the action is available. `Toast.tsx`'s `action` prop (Retry/Undo) sat unreachable this way for ~4 months before any call site actually exercised it (`docs/solutions/logic-errors/toast-action-button-unreachable-by-screen-reader-2026-07-13.md`) — a newly-activated dormant prop on a shared component is exactly the case most likely to hide this, since no prior usage ever screen-reader-tested it
- [ ] **A reused component carrying its OWN `accessible` container `accessibilityLabel` is swallowed too — not just decorative badges (above) or interactive descendants (Toast case).** A self-labeling _informational_ component (e.g. `RecipeAllergenLabel`, whose container composes `"Contains: Peanuts, Tree Nuts"`) placed inside a card `Pressable` (which defaults `accessible={true}` + sets its own label) has its informational label dropped from the a11y tree — the parent focus stop never walks into it, so the visible chips still render (a sighted-only check passes) while a screen-reader user hears only the card's base label. For **safety-relevant** content this is a silent safety regression, not cosmetic. Require the parent to compose the child's info into its own `accessibilityLabel` by reusing the child's pure label util (e.g. `toRecipeAllergenLabels`), never a client-side re-derivation, while keeping the visible component for sighted users. See `docs/solutions/conventions/precautionary-safety-display-renders-nothing-never-safe-2026-07-24.md` and `docs/solutions/conventions/parent-label-prefix-decorative-children-2026-05-13.md`
- [ ] **An `accessibilityLabel` on a React Native `<Image>` is the wrong place for it, and BOTH outcomes are bad — flag the placement, not just a missing label.** RN gates on `accessible={props.alt !== undefined ? true : props.accessible}`, _identically_ in `Image.ios.js` and `Image.android.js`, so a bare `accessibilityLabel` does not make the image an accessibility element on either platform and may never be announced; if a platform heuristic surfaces it anyway it then double-announces against an adjacent visible caption ("Nutrition label you photographed", then "Nutrition label"). **Ask the intent question BEFORE reaching for a wrapper (2026-08-04, PR #751): is the image decorative?** If its label merely restates adjacent visible text (product name beside an `<h2>` of that name, avatar beside "Hello {name}"), the correct fix is to DELETE the label and mark the image decorative — making it announce would introduce a double-announcement, so the inert-label bug was accidentally masking a second bug. Four call sites of `FallbackImage` were in exactly this shape. Only when the image genuinely carries information no neighbouring text does should the label go on an `accessible` group wrapper collapsing image + caption into one node — subject to the usual two conditions: no interactive child in the group, and the group label must CONTAIN the caption's words (a collapsed subtree announces only the group label and silently drops nested text). **Containment decides the direction in general:** collapse a group only when its label is a SUPERSET of what its children say; when the children are RICHER than any summary (per-badge explanations vs. a one-line summary), drop the group instead — iOS's collapse otherwise makes that detail unreachable. Note the jsdom harness cannot see `accessible` in either direction, so passing tests are never evidence here — this is a read-the-code check. The group-wrapper shape is **device-confirmed on iOS VoiceOver (2026-07-31)**: one announcement per tile, next swipe reaching the sibling photo rather than the caption — so treat a diff that moves the label back onto the `<Image>` as a regression against a measured result, not a style preference. **Android/TalkBack IS now verified (2026-08-04):** the bare-label image renders `content-desc` set with `focusable=false` in the real Android tree — a description TalkBack skips — and an RN `accessible={true}` wrapper does NOT collapse its subtree on Android at all (wrapper AND every child stay focusable, so a 3-badge group is 4 stops there against 1 on iOS). (Ref: `docs/solutions/design-patterns/accessibility-grouping-pattern-2026-05-13.md` → "Labelling an image"; `docs/solutions/design-patterns/remove-an-inert-prop-from-the-public-type-2026-08-04.md`)
- [ ] **`importantForAccessibility="no"` is NOT a weaker `"no-hide-descendants"` — it is a different SCOPE.** `"no"` excludes only the view it is set on; the subtree stays in the Android a11y tree, and a child's own `accessible={false}` does not save you. Flag `"no"` on any container that has children (an icon glyph renders as a `Text` node holding a private-use codepoint and gets announced raw). Require `"no-hide-descendants"` + `accessibilityElementsHidden` for the iOS half. **Do not accept `focusable=false` from a `uiautomator` dump as evidence of exclusion** — ordinary body text is `focusable=false` and is certainly announced; only `uiautomator dump --compressed` answers reachability. See `docs/solutions/conventions/a11y-hide-visually-hidden-surfaces-2026-06-10.md`
- References: `client/screens/ProfileScreen.tsx` (SettingsItem), `client/components/home/ActionRow.tsx`, `client/components/Toast.tsx`, `client/screens/NutritionDetailScreen.tsx` (captured-photo tiles)
...
9. **A suppression flag whose value cannot change within a screen's lifetime is a configuration switch, not a guard — and in staged work it usually guards a hazard a later task made unreachable.** Shipped and caught in review (slice 2c, PR #753): `suppressAnnounce={logGate.kind === "needsAcknowledgement"}` existed to stop the notice announcer colliding with an acknowledge announcement, since iOS `UIAccessibility.post(.announcement)` does not queue. Two tasks later the acknowledgement state moved into the button component, so the click re-renders only that component — the announcer's effect never re-runs and the collision became unreachable; ordering alone already made "acknowledge wins" true (the click handler fires later, and the later post silences the earlier). The guard bought nothing and cost the announcement on **exactly the screen it mattered on**: the log gate fires when a photographed label could not be used, which is the same condition that sets `labelReadNotice`, and since the same change deleted the `accessibilityLiveRegion="polite"` regions (Android-only), the imperative announcer had just become Android's ONLY signal. Reviewer red flags: every announcer assertion is a negative or a count-of-one on the SUPPRESSED fixture (a permanently-on flag passes the whole suite — demand the positive/unsuppressed test); a coordination prop between two components that no longer re-render together; a guard preserved "just in case" with nobody able to write down the reachable path that still needs it. When a task relocates state, require its report to name which coordination mechanisms the move makes unnecessary — a diff showing only the move cannot reveal this. (Ref: `docs/solutions/logic-errors/a-guard-outlives-the-state-layout-it-was-written-for-2026-08-04.md`)
...
- [ ] **`focusTo()` must be passed an explicit `modes` array on iOS ONLY** — derived from `device.supportsExposureMetering` / `supportsFocusMetering` / `supportsWhiteBalanceMetering` via `supportedMeteringModes()` (`client/camera/hooks/useCameraFocusAndZoom-utils.ts`), and guarded against `[]` (iOS throws `"MeteringModes cannot be empty!"`). On the pinned 5.0.11, AVFoundation appends `.awb` unconditionally, so one unsupported mode fails the WHOLE metering operation (AF included) and tap-to-focus draws its ring while never refocusing. **Never extend this to Android** — CameraX already derives modes from `isFocusMeteringSupported()` for the _actual tapped point_, which is strictly better than our point-agnostic device flags; overriding it downgrades a correct check on a platform that never had the bug. Don't hardcode `['AF']` either — that fixes one handset while dropping AE/AWB everywhere else. Fixed upstream in 5.1.0 (#3976); delete the branch when that bump ships. See `docs/solutions/logic-errors/library-auto-capability-default-fails-whole-operation-2026-07-25.md`
- [ ] **Flag `.catch(() => {})` on any camera/native promise, and flag `logger.warn` used as release-build instrumentation.** A JS-drawn affordance (focus ring, zoom label) renders identically whether the native call succeeded, rejected, or was swallowed — so an empty catch turns a native failure into a feature that looks like it works, and JS ships over OTA while native does not. `logger.warn`/`.info` are `__DEV__`-only (`client/lib/logger.ts`); diagnostics meant to be read from a `preview`/`production` build must use `logger.error` (→ Sentry), latched once per mount when it fires from a gesture handler. See `docs/solutions/conventions/js-rendered-feedback-not-evidence-native-call-succeeded-2026-07-25.md`
...
- [ ] **`ListEmptyComponent` renders purely off `data.length === 0`** — it has no awareness of any other local state. An action inside it that flips a state flag which swaps a _sibling_ section (footer button → inline form, etc.) does NOT hide the empty-state's own buttons; they stay on screen above/beside the surface they handed off to, and re-tapping them is a confusing no-op. Condition every `ListEmptyComponent` action (`actionLabel`/`onAction`, secondary variants) on that same flag. Reference: `client/components/CookbookPickerModal.tsx` (`showNewInput`), `docs/solutions/logic-errors/flatlist-listemptycomponent-ignores-sibling-ui-toggle-2026-07-14.md`
