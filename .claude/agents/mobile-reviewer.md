...
- [ ] **"These two states are mutually exclusive" proven by enumerating ASSIGNMENT SITES is only true within one invocation — check for a reset before accepting it.** A per-call-site sweep answers "can these be _assigned_ together?"; what decides whether a user hits the bug is "can these be _true_ together?", and for state with no reset that spans every later call on the same hook instance. Two reviewers reached OPPOSITE conclusions on the same code from this: one verified per call site that `correctionNotice`/`isPer100g` are set only in success branches disjoint from every `setError`, which is correct for a single lookup; a probe against the real unmocked hook over TWO sequential lookups found the combination reachable, because nothing resets either one. **The reset block's own asymmetry is the tell and sits in the same function** — `fetchBarcodeData` explicitly re-arms eight fields at the top (`flags`, `conflict`, `labelReadNotice`, `labelUsed`, `dbSnapshot`, `activeSource`, `isBeverage`, `validatedData`), each with a fail-safe rationale, and silently omits these two; there is no `setCorrectionNotice(null)` anywhere in the file. So a label RETAKE carries a stale correction into a lookup that errors and both `NoticeStack` and `InlineError` announce in one commit — two utterances, and iOS drops the first. Cheapest check: grep the setter; if only assignments come back, the value is monotone for the instance's lifetime. A content-keyed `lastAnnouncedRef` does not absorb this — suppressing one contributor makes the composed key shorter, not absent. (Ref: `docs/solutions/conventions/mutual-exclusion-proven-per-call-site-can-co-occur-across-invocations-2026-08-06.md`)
