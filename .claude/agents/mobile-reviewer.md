...
- [ ] **`focusTo()` must be passed an explicit `modes` array derived from the device's own capability flags** — never rely on its documented "all supported modes" default. On the pinned 5.0.11, a request containing one unsupported mode fails the WHOLE metering operation (AF included), so tap-to-focus draws its ring and never refocuses. Build the array from `device.supportsExposureMetering` / `supportsFocusMetering` / `supportsWhiteBalanceMetering` (`supportedMeteringModes()` in `client/camera/hooks/useCameraFocusAndZoom-utils.ts`); do not hardcode `['AF']` — that fixes one handset while silently dropping AE/AWB metering everywhere else. Fixed upstream in 5.1.0 (#3976); revisit only after that bump ships. See `docs/solutions/logic-errors/library-auto-capability-default-fails-whole-operation-2026-07-25.md`
