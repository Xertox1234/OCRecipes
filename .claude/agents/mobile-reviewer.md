...
- [ ] **"These two states are mutually exclusive" proven by enumerating ASSIGNMENT SITES is only true within one invocation — check for a reset before accepting it.** A per-call-site sweep answers "can these be _assigned_ together?"; what decides whether a user hits the bug is "can these be _true_ together?", which for unreset state spans every later call on the same hook instance. Two reviewers reached OPPOSITE conclusions here and both were internally right: per call site, `correctionNotice`/`isPer100g` are set only in success branches disjoint from every `setError` — true for ONE lookup; a probe over TWO sequential lookups found the combination reachable, because nothing resets either. **The reset block's own asymmetry is the tell, in the same function** — `fetchBarcodeData` explicitly re-arms eight fields at the top, each with a fail-safe rationale, and omits these two; there is no `setCorrectionNotice(null)` in the file. A label RETAKE then carries a stale correction into an errored lookup, so `NoticeStack` and `InlineError` both announce in one commit and iOS drops the first. Cheapest check: grep the setter — if only assignments come back, the value is monotone for the instance's lifetime. A content-keyed `lastAnnouncedRef` does not absorb it: suppressing one contributor makes the composed key shorter, not absent. (Ref: `docs/solutions/conventions/mutual-exclusion-proven-per-call-site-can-co-occur-across-invocations-2026-08-06.md`)
