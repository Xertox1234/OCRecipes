...
- [ ] **An `accessibilityLabel` on a React Native `<Image>` is the wrong place for it, and BOTH outcomes are bad — flag the placement, not just a missing label.** RN gates on `accessible={props.alt !== undefined ? true : props.accessible}`, _identically_ in `Image.ios.js` and `Image.android.js`, so a bare `accessibilityLabel` does not make the image an accessibility element on either platform and may never be announced; if a platform heuristic surfaces it anyway it then double-announces against an adjacent visible caption ("Nutrition label you photographed", then "Nutrition label"). Require the label on an `accessible` group wrapper collapsing image + caption into one node — subject to the usual two conditions: no interactive child in the group, and the group label must CONTAIN the caption's words (a collapsed subtree announces only the group label and silently drops nested text). Note the jsdom harness cannot see `accessible` in either direction, so passing tests are never evidence here — this is a read-the-code check. The group-wrapper shape is **device-confirmed on iOS VoiceOver (2026-07-31)**: one announcement per tile, next swipe reaching the sibling photo rather than the caption — so treat a diff that moves the label back onto the `<Image>` as a regression against a measured result, not a style preference. Android/TalkBack is unverified (no Android build exists). (Ref: `docs/solutions/design-patterns/accessibility-grouping-pattern-2026-05-13.md` → "Labelling an image")
