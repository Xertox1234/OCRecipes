...
- [ ] **An inferred field must not count toward `confidence`.** Sharper than the "confidence is not a gate" item above: `confidence` gates whether `LabelAnalysisScreen` shows the instant preview, so letting inference raise it makes the gate self-fulfilling — the inference authorises showing the inference. Populating a field and vouching for a field are separate outcomes; flag any recovery path that increments `extracted`. Also check what reconciles the recovered field: `shouldReplaceWithAI` compares only calories/fat/protein/carbs/sodium, so an inferred `saturatedFat` or `dietaryFiber` is never re-checked by the AI pass. **`buildLabelConflict` DOES corroborate `saturatedFat`, but ONLY a directly-read one** (PR #764). An earlier revision excluded the field entirely and called that deliberate; that was overridden. It is now in `cmp` (`server/services/label-override.ts`) with **three** policies attached, all of them properties of the row rather than parallel lists:
  1. `requiresDirectRead: "saturatedFat"` — the comparison runs only when the payload's `directReads` positively lists the field. The parser (`client/lib/nutrition-ocr-parser.ts`) can INFER `saturatedFat` from its `totalFat` parent via `gluedUnitIsForced`, and that inference's error is bounded by `[0, totalFat]` — orders of magnitude wider than the print-rounding error the floor is sized for. Provenance gates the comparison ONLY; an inferred value is still accepted and still adopted.
  ...
  Review against THAT shape. Flag: a change widening `PARENT_FIELD`/`gluedUnitIsForced` to a weaker inference for this field; a floor changed to a value not traceable to a printing rule, or an unclamped one; `corroborates` flipped to `true` without re-arguing both of its meanings; and above all **any provenance encoding where ABSENT could mean "direct"** — the field must stay optional and absence must fall to "do not compare", or every already-installed client silently keeps the old behaviour.
  ...
  **And check what the narrowing re-exposes.** Scoping the blanking made a second-order defect reachable that the coarse version had been suppressing: retaining the record's `fat` beside a label-corrected `saturatedFat` can display `saturatedFat > fat`, which is nutritionally impossible. `retainableSiblings` drops the parent only when the invariant would actually be violated. The general check for any "blank less" / "keep more" change: for each value now retained, is it in a CONTAINMENT relationship (`saturated`/`trans ≤ fat`, `sugars ≤ carbs`) with a value the label just replaced? Mixed provenance across such a pair is the failure, and `LabelNutritionInput`'s missing fields are what make it possible — the parser can decline a glued `totalFat` while reading `Saturated Fat 6 g` on the next line perfectly.
