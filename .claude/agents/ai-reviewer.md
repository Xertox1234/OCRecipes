...
- [ ] **An inferred field must not count toward `confidence`.** Sharper than the "confidence is not a gate" item above: `confidence` gates whether `LabelAnalysisScreen` shows the instant preview, so letting inference raise it makes the gate self-fulfilling — the inference authorises showing the inference. Populating a field and vouching for a field are separate outcomes; flag any recovery path that increments `extracted`. Also check what reconciles the recovered field: `shouldReplaceWithAI` compares only calories/fat/protein/carbs/sodium, so an inferred `saturatedFat` or `dietaryFiber` is never re-checked by the AI pass. **`buildLabelConflict` DOES corroborate `saturatedFat` as of P1-2026-08-05-saturated-fat-overrides-db-without-corroboration** — an earlier revision of that branch excluded it and called the exclusion deliberate; that was overridden and the field is in `cmp` with two policies attached (`server/services/label-override.ts`): a quantization floor of `0.5 g × factor`, derived from the FDA/CFIA 0.5 g printing step scaled by the same per-serving → per-100 factor the reading itself is scaled by (so rounding alone cannot fire a conflict in the 2-5 g/100g band), and `corroborates: false`, so its agreement never counts toward `comparedCount` or the `compared >= 2` one-tap-log gate. Review against THAT shape: flag a change that widens `PARENT_FIELD`/`gluedUnitIsForced` to a weaker inference for this field; flag a floor changed to a value not traceable to a printing rule; flag `corroborates` being flipped to `true` without re-arguing why a comparison run at a widened tolerance should be allowed to open a trust gate. The residual that remains: a wrong `saturatedFat` still rides into `mergedPer100g` when the RECORD has no `saturated-fat_100g` to compare against and another field conflicts. (Ref: `docs/solutions/logic-errors/confidence-must-count-evidence-not-inferences-2026-08-05.md`)
