...
**Merge-gate output contract (machine-parsed — applies whenever you are dispatched with `REVIEWED-SHA` / `REVIEWED-FILES` blocks).** `.claude/hooks/review-stamp-writer.sh` parses your reply into a review record, so three shape rules are load-bearing. (1) Put any correctly-implemented-patterns or context notes list BELOW the `REVIEWED-FILES` block and ABOVE your final line, never among the paths — the gate reads that block from the label to the first line of CONTENT containing a space, treating a blank or whitespace-only line as skipped rather than as a terminator, so a prose line among the paths is digested as a changed file and the merge is denied for mis-scoped review. (2) The first line of content below the file list must CONTAIN A SPACE — a bare token like `Findings:`, `Notes:` or `---` is indistinguishable from a path and is digested as one. (3) Keep the three bracketed severity words out of everything except real finding lines — the notes prose, every other part of your reply, and above all any short WRAPPER LINE you write after handing the report back; not even to say there were none, and not even to say that findings you reported earlier are now resolved. Detection deliberately over-detects and scans both surfaces, with DIFFERENT REACH and different outcomes: the HIGHEST of the three sets verdict `findings` only on a REPORT line that STARTS with the tag or a file:line citation (prose no longer does, since 2026-09-23), while ANY of the three ANYWHERE in the HAND-BACK WRAPPER still writes no record at all and denies the merge for a finding that does not exist. Write `no blocking issues` instead, keep those words for real finding lines, and keep the wrapper plain: `Review complete and handed back; no blocking issues.` Measured 2026-09-20: of 16 clean hand-backs in one session, the only one that wrote no record was a confirmation round whose wrapper listed the findings it had just verified as fixed. Report the sha you actually observed, never one you were told.
...
- [ ] **When a resolution rule IS permitted, its reference values must be as trustworthy as the rule pretends.** The decline-by-default rule above has narrow exceptions — cases the label's own content makes impossible (an unprintable leading zero; a child exceeding its parent). Those are sound, but a rule reading `result[otherField]` is reasoning over a **reading**, not a fact, and the other field may have come through the same tolerance being adjudicated. `!= null` is an existence check, not a trust check: `"Total Fat 129 9%"` parses `totalFat` 129 via the spaced `g`→`9` branch (the `9` is a `%`-stripped daily value), and containment then forced `"Saturated 259"` to 25 because 259 > 129. Require the rule to record HOW each value was obtained and refuse derivations it cannot lean on. Two ambiguous readings do not add up to one certain one. (Ref: `docs/solutions/logic-errors/derived-bound-is-only-as-trustworthy-as-its-derivation-2026-08-05.md`)
- [ ] **A cross-field invariant over label data must name its jurisdiction, and OFF is never single-jurisdiction.** `dietaryFiber ≤ totalCarbs` is a fact about US labelling, not about nutrition: EU 1169/2011 declares AVAILABLE carbohydrate with fibre listed separately outside it. On an EU-sourced bran/psyllium/chia record a correct fibre reading legitimately exceeds a correct carbohydrate one, and a containment rule "corrects" it to a wrong value rather than dropping it. `totalSugars ≤ totalCarbs` and `saturated/trans ≤ totalFat` are sound everywhere (and survive the FDA/CFIA rounding grid, which is monotonic). Distrust: carbs vs fibre, salt vs sodium, kJ vs kcal, sugars vs added sugars. This repo has been bitten by the fibre case before — the Atwater check excludes fibre for the same reason — so grep the solutions corpus for a new rule's ENTITIES before approving it. (Ref: `docs/solutions/logic-errors/regime-dependent-invariant-breaks-on-mixed-provenance-data-2026-08-05.md`)
- [ ] **An inferred field must not count toward `confidence`.** Sharper than the "confidence is not a gate" item above: `confidence` gates whether `LabelAnalysisScreen` shows the instant preview, so letting inference raise it makes the gate self-fulfilling — the inference authorises showing the inference. Populating a field and vouching for a field are separate outcomes; flag any recovery path that increments `extracted`. Also check what reconciles the recovered field: `shouldReplaceWithAI` compares only calories/fat/protein/carbs/sodium, so an inferred `saturatedFat` or `dietaryFiber` is never re-checked by the AI pass. **`buildLabelConflict` DOES corroborate `saturatedFat`, but ONLY a directly-read one** (PR #764). An earlier revision excluded the field entirely and called that deliberate; that was overridden. It is now in `cmp` (`server/services/label-override.ts`) with **three** policies attached, all of them properties of the row rather than parallel lists:
  1. `requiresDirectRead: "saturatedFat"` — the comparison runs only when the payload's `directReads` positively lists the field. Provenance gates the comparison ONLY; a value kept out of the list is still accepted and still adopted. **TWO parser paths produce a value that is not a direct read, and both must stay out of the list:** (a) the second pass, where `gluedUnitIsForced` INFERS `saturatedFat` from its `totalFat` parent — error bounded only by `[0, totalFat]`; (b) the first pass, where the match's SECOND group is set, meaning the token that should have been the "g" unit was recognised as a spaced digit. (b) is the easier one to get wrong because the value is otherwise an ordinary first-pass read: `Saturated Fat 29 9%` (from a plain `Saturated Fat 2g 9%` line) adopts 29 for a package printing 2, a 14.5x inflation. Marking it `substitutedUnit` so it cannot BOUND another field, while still publishing it as a direct read so it can CONDEMN a record, is half a fix — the two consumers need the same judgement.
  2. A quantization floor of `0.5 g × factor`, derived from the FDA/CFIA 0.5 g printing step scaled by the same per-serving → per-100 factor the reading is scaled by (so rounding alone cannot fire a conflict in the 2-5 g/100g band), **clamped** to half the MEDIUM band width in `shared/constants/nutrition-bands.ts` (1.75 food / 0.875 drink) so a tiny serving cannot inflate it past an FSA band crossing.
  3. `corroborates: false`, so its agreement never counts toward `comparedCount` or the `compared >= 2` one-tap-log gate — and, the second meaning, so its disagreement alone does not trigger the sibling blanking (see below).

  Review against THAT shape. Flag: a change widening `PARENT_FIELD`/`gluedUnitIsForced` to a weaker inference for this field; a floor changed to a value not traceable to a printing rule, or an unclamped one; `corroborates` flipped to `true` without re-arguing both of its meanings; any first-pass path that pushes to `directReads` UNCONDITIONALLY (`confidence` and `directReads` count different things on purpose — `directReads.length <= extracted`, and a comment claiming they are one set is the tell); and above all **any provenance encoding where ABSENT could mean "direct"** — the field must stay optional and absence must fall to "do not compare", or every already-installed client silently keeps the old behaviour.

  **The blanking-scope rule, which generalises past this field:** a conflict makes `buildLabelConflict` discard the record's un-read macros (carbs/protein/fibre/sodium). That is justified only for a _corroborating_ field's disagreement, which implies the record's whole per-100 basis is wrong (the Cherry Coke shape). A `saturatedFat`-only mismatch with calories/sugar/fat all AGREEING is evidence _against_ a basis-wide error, so `basisDisproven` scopes the blanking to conflicts that include a corroborating field. Whenever a review widens the set of inputs that can trigger a destructive consequence, check that the consequence's own rationale covers every new trigger — the tell is an unconditional action justified by a field-specific argument.

  **And check what the narrowing re-exposes — against the TABLE, not against prose.** Scoping the blanking made a second-order defect reachable that the coarse version had been suppressing: displaying `saturatedFat > fat`, which is nutritionally impossible. The guard is `enforceContainment` + `CONTAINMENT_PAIRS` in `server/services/label-override.ts`, and the pairs are exactly:
  ...
  That set is the client parser's `PARENT_FIELD` (`client/lib/nutrition-ocr-parser.ts`) in this module's vocabulary. **`fiber`/`carbs` is excluded on purpose** — EU 1169/2011 declares AVAILABLE carbohydrate with fibre listed outside it while US labels count fibre inside, and OFF aggregates both, so a correct EU fibre figure legitimately exceeds a correct carbohydrate one. Flag any change that adds it, and flag a pair added to one side of the wire but not the other.

  Three properties to check, each of which was once wrong here:
  - **Evaluated on the FINAL MERGED block**, not pairwise against the label's readings and the record's. A pairwise form can look correct purely because some field is absent from today's payload; adding a field to `LabelNutritionInput` then breaks it silently.
  - **No "the label supplied both sides, so it is self-consistent" carve-out.** `totalFat` and `saturatedFat` are INDEPENDENT PER-LINE CAPTURES. `Total Fat 5g 6%` / `Saturated Fat 2g 9%` with the g→9 misread yields `saturatedFat: 29` beside a perfectly-read `totalFat: 5` — 96.7 vs 16.7 g/100g at a 30 g serving, both label-sourced, and no mixed provenance anywhere. The guard runs on BOTH merge branches (blanking and retaining) for exactly this reason.
  - **Provenance decides which SIDE is dropped, never whether to look.** Record value contradicted by a label value → drop the record's. Both from the label → drop the CHILD (it is the reading that exceeds its own definitional bound). Neither from the label → leave both: that is the record contradicting itself, the merge did not cause it, the plain lookup path displays it untouched, and dropping the child would delete the FLAG-BEARING side (`sugar`/`saturatedFat` carry FSA flags; `carbs`/`fat` carry none).

  **The generalisable rule, and the reason this section is a table.** A stated principle and a single-instance implementation written in the SAME COMMIT still diverged: the previous revision of this very item named `saturated`/`trans ≤ fat` and `sugars ≤ carbs` in prose while the code guarded one pair, and `sugar > carbs` shipped unguarded into a food log. **A rule that names N cases needs a table of N rows, not a conditional for the one that was reproduced first** — and the review check is to diff the rule's enumeration against the code's enumeration, not to read the prose and nod.

  Residuals that remain: a wrong `saturatedFat` still rides into `mergedPer100g` whenever it was not compared (no `saturated-fat_100g` on the record, an inferred reading, or an older client sending no provenance) and another field conflicts; and on the non-blanking path `compared: true` can open one-tap logging over DB-sourced macros that were never checked. (Ref: `docs/solutions/logic-errors/confidence-must-count-evidence-not-inferences-2026-08-05.md`)
...
- [ ] Presence-detection for a NEGATABLE attribute (caffeine / gluten / dairy / any "free-from") must handle the NEGATED form. A bare substring/keyword matcher fires on the negation because the token sits inside it — `koffein` ⊂ `koffeinfrei`, `caffeine` ⊂ `caffeine-free`, `cafeína` ⊂ `sin cafeína` — and an explicit numeric `0` or a broad category (decaf coffee is still `en:coffees`) reads as presence. Require an explicit negation/decaf suppressor **plus** strictly-`> 0` numeric checks. Word boundaries alone do NOT fix it (`\bcaffeine\b` still matches `caffeine-free`; `\b` misbehaves around accents). Reviewer red flag: a presence matcher for a negatable attribute whose test set contains **no negated fixture in each supported language** — that suite is structurally blind to the false-positive direction. (Ref: PR #694; `docs/solutions/logic-errors/presence-matcher-false-flags-negated-free-from-form-2026-07-22.md`)
...
- [ ] A branch that accepts a sentinel value as trustworthy (explicit 0 kcal = "true zero", empty string = "intentionally blank") must (a) NOT sit behind guards only the general path needs — inherited preconditions silently un-fix the special case for inputs that differ from the motivating repro — and (b) cross-check the record's redundant encodings for contradiction before trusting the sentinel (kcal vs kJ fields, energy vs Atwater-from-macros: `0 === 0` between two defaulted fields is not evidence). Both failure modes shipped in one branch of `offSelfConsistent` (PR #656). (Ref: `docs/solutions/logic-errors/explicit-zero-corroboration-needs-contradiction-checks-2026-07-17.md`)
- [ ] A feature that OVERRIDES a subset of a nutrition record judged untrustworthy (scanned label over a wrong DB entry; corrected sub-fields) must build the corrected result from ONLY the trusted-source fields and BLANK (leave `undefined` — never inherit, never `0`) every sibling field the source did not provide. A record detected as _uniformly_ wrong has no trustworthy field, so a `{ ...dbResult, ...corrections }` merge re-introduces the error in the un-corrected fields AND breaks cross-field invariants — the shipped bug was sugar 39 g > total carbs 11 g after correcting only calories/sugar/fat. Carrying "more parsed fields through" is NOT the fix: OCR frequently drops fields (the flagship label had no carbs line), so the sibling stays DB-wrong. Blocker red flags: a spread whose base is the record being distrusted; a wrong-record test fixture that OMITS the sibling fields (so the impossibility never surfaces — require a fixture that sets a sibling like `carbs` and asserts the numeric invariant `!(sugar > carbs)`, plus a partial-input case). Also verify the override's decision guard cross-checks any input-derived scaling factor (OCR serving grams) against a _trusted_ anchor before trusting the computed comparison, failing toward the DB on gross disagreement. (Ref: PR #695; `docs/solutions/logic-errors/partial-correction-must-blank-untrusted-sibling-fields-2026-07-22.md`, `docs/solutions/design-patterns/cross-check-derived-factor-against-trusted-anchor-2026-07-22.md`)
- [ ] A data-provenance self-consistency gate keyed on ONE corroboration field (e.g. per-serving energy) fails OPEN for every record LACKING that field — before trusting the gate as protection, ask what fraction of REAL records carry it. When the field-bearing records are the minority (most of OFF has only per-100g, no per-serving energy), the gate silently degrades to "no protection" for the majority and a name/similarity-matched secondary replaces identity-matched data unchecked (Nutella `3017620422003`: 539→182 kcal, wrong sugar → FSA `nutrient:sugar` silently suppressed). Fix shape: supply a FALLBACK corroboration from a DIFFERENT independent field set (here `offMacrosCorroborateEnergy`, an Atwater `4p+4c+9f`-vs-stated-energy check) for records missing the primary field. Reviewer red flags: a "self-consistent" gate whose only positive path requires a field the corpus mostly lacks; a fix that widens a first-write-wins cache's poisoned population without a remediation follow-up. Nutrition gotcha: an Atwater/energy-vs-macros check on OFF data must NOT add or subtract fiber — OFF mixes EU (carbs EXCLUDE fibre) and US (carbs INCLUDE fibre) labels, so any fiber-correction sign is wrong for half the corpus; ignoring fiber is minimax-robust (worst-case 2·fiber vs 4·fiber). Verify a proposed convention-dependent correction against the data source's ACTUAL mixed provenance before applying it. (Ref: PR for P2-2026-07-22; `docs/solutions/logic-errors/name-matched-secondary-must-not-replace-self-consistent-label-2026-07-17.md` → Refinements 2026-07-24)
- [ ] **Two nutrition objects that are field-parallel on the PRIMARY path are assembled independently on the FALLBACK path — never read one and reason about the other.** `perServing = scaleNutrients(per100g, scale)` on the server path makes the assumption look safe; the direct-OFF fallback builds `perServing` field-by-field from independently-present OFF fields (`sugars_serving` etc.) while `per100g` comes from the `_100g` fields, and the plausibility gate checks **calories only** — so `per100g.sugar = 11.0` with `nutrition.sugar = undefined` is a common real shape. Four instances shipped in ONE branch (slice 2c, PR #753), all found by review, none by a 2730-test suite, because each half was internally consistent and every fixture set both objects equal. Specific traps: (a) a serving string read from display state that `recalculateNutrition` rewrites, rather than from the invariant `validatedData.servingInfo.displayLabel` — this silently swaps the whole FSA threshold table when `isBeverage` is absent, since `resolveBasis` infers drink-vs-food from the string's unit; (b) a filter that drops a server warning because a panel "owns" that nutrient, when the panel can only replace it if the basis resolves; (c) **band EXISTENCE vs band AGREEMENT** — the client passes no `portionGrams`, so it can never reproduce `universal-flags.ts`'s FSA per-portion HIGH override; dropping a `warn` badge on a MEDIUM band deleted "High in sugar" on Cherry Coke (355 mL, 39 g → per-100 10.99 vs the drink HIGH line 11.25), and on a 700 g food the colour inverts to a green LOW where the red warning was; (d) `hasValue` derived from the band source alone, so a standout promotes "Moderate sugar" while the panel row shows "Not recorded". Reviewer red flags: a deletion of a `severity: "warn"` health signal justified by "the other surface carries it now" without an enumeration of the states where it does not; a docblock asserting two objects are equivalent without naming the construction that makes them so; test fixtures that set both sources to the same value (they cannot distinguish buggy from fixed). Under-warning by declining to invent a signal is permitted; **discarding a real server-computed signal is not** — say which direction a change takes. (Ref: `docs/solutions/logic-errors/field-parallel-objects-diverge-on-the-fallback-path-2026-08-04.md`)
