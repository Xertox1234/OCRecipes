...
- [ ] **An inferred field must not count toward `confidence`.** Sharper than the "confidence is not a gate" item above: `confidence` gates whether `LabelAnalysisScreen` shows the instant preview, so letting inference raise it makes the gate self-fulfilling — the inference authorises showing the inference. Populating a field and vouching for a field are separate outcomes; flag any recovery path that increments `extracted`. Also check what reconciles the recovered field: `shouldReplaceWithAI` compares only calories/fat/protein/carbs/sodium, so an inferred `saturatedFat` or `dietaryFiber` is never re-checked by the AI pass. And note `saturatedFat` is the one payload field `buildLabelConflict` never corroborates — its `cmp` list is calories/sugar/fat, so it rides into `mergedPer100g` on another field's conflict. **This has since been reviewed and decided (P1-2026-08-05-saturated-fat-overrides-db-without-corroboration): the exclusion is deliberate, not an open gap** — the server can't condition on OCR provenance once the payload arrives (direct read, inference, and plain misread are indistinguishable), so no comparison could trust one saturatedFat reading over another. Do not re-flag this as unaddressed; the reasoning is documented in the comment above `cmp` in `server/services/label-override.ts`. DO flag a future change that widens `PARENT_FIELD`/`gluedUnitIsForced` to a WEAKER inference for this field, or that reintroduces `saturatedFat` into `cmp` without re-examining the same tradeoffs. (Ref: `docs/solutions/logic-errors/confidence-must-count-evidence-not-inferences-2026-08-05.md`)
