...
19. **Unquoted `:var` psql substitution never validated as numeric** — A bash CLI script (any `scripts/pg-lab/*.sh`-style tool, or future raw-`psql` tooling) that takes a "numeric" flag (`--min-support`, `--limit`, `--threshold`) and passes it straight into `-v name="$value"`, referenced **unquoted** in the SQL (`col >= :name`, no surrounding `'`), executes it as literal SQL text — not a bound parameter. `--min-support "0; INSERT INTO canary VALUES (1)"` runs a stacked statement; `--min-support "5 OR TRUE"` silently rewrites the WHERE clause with no psql error at all (the query stays syntactically valid throughout, so this is invisible to error-based testing — verify by checking RESULTS, not exit codes). Only the quoted `:'var'` form is psql-side-escaped and safe for arbitrary string content; the bare form is required for genuinely numeric contexts but provides zero protection on its own. Demand `[[ "$value" =~ ^[0-9]+$ ]] || exit 1` (or a fixed keyword enum) validating the shell variable BEFORE it reaches `-v`, for every bare `:var` reference in a diff — grep for `-v \w+="\$` paired with a non-`'`-wrapped `:name` in the accompanying SQL (Ref: `docs/solutions/logic-errors/psql-c-flag-skips-var-substitution-2026-07-05.md`, `scripts/pg-lab/git-mine.sh`)
20. **Database-name (or hostname/identifier) denylist parsed by naive string-slicing off a connection string** — A "must never connect to the real app database" refusal check that extracts the comparison value via `str.slice(str.lastIndexOf("/") + 1)` (TS) or `${VAR##*/}` (bash) and then does exact-string equality is bypassed by a trailing query string (`postgresql://host/nutricam?sslmode=require` slices to `"nutricam?sslmode=require"`, which fails `=== "nutricam"` while `pg`/`psql`'s real parser still connects to `nutricam`) — and, if the fix loosens to an identifier-shaped allowlist alone, by percent-encoding (`nutr%69cam`). **Even the correct-looking fix (query/fragment strip + identifier-format allowlist) is still bypassable by a `?dbname=` query-parameter override** — `postgresql://host/ocrecipes_lab?dbname=nutricam` strips to the safe-looking, identifier-shaped `ocrecipes_lab`, but libpq honors the `dbname` query parameter over the URI path segment and connects to the real `nutricam` anyway (live-verified 2026-07-07 against a real Postgres). Any hand-parsed defense — however many special cases it strips — is chasing libpq's real parser one bypass behind. The only fully robust fix is to stop hand-parsing the URI at all: connect, then ask the driver for ground truth (`SELECT current_database()` in bash via `psql -tAqd "$URL" -c '...'`; `client.query('SELECT current_database()')` in TS/`pg`) and denylist-match THAT result, not any string derived from the connection string. When the same denylist pattern is copied across sibling scripts, a fix to one copy does not protect the others — check every copy (Ref: `docs/solutions/logic-errors/denylist-bypassed-by-connection-string-query-string-2026-07-06.md`)
21. **Unpinned `npx --yes <tool>` execution in scripts/hooks/CI** — a script that shells out to `npx --yes knip`/`jscpd`/`madge` (or any registry tool not in `package.json`) downloads and executes whatever version the registry serves as _latest_ at run time: an upstream compromise executes arbitrary code with the developer's local credentials (git, `.env`, npm token) during a routine run. Registry versions are immutable, so demand an exact pin (`npx --yes knip@6.26.0`, ideally via a single exported pin constant like `NPX_PINS` so the executed pin and any documented re-run command can't diverge) or a lockfile-pinned devDependency. **A top-level `@<semver>` pin only freezes the named package itself — its transitive dependencies still resolve unconstrained at fetch time (no lockfile), so a compromised patch release of a transitive dep still executes.** Only a lockfile-pinned devDependency freezes the whole tree; treat an `NPX_PINS`-style top-level pin as raising the bar, not closing the supply-chain gap. `--yes` without a version is the grep: `npx (--yes |-y )?[a-z@][^@ ]+( |$)` with no `@<semver>` (Ref: `scripts/audit-scanners.ts` review 2026-07-12)
22. **Command-safety gate under-models a tool's option grammar** — a hook/gate that decides whether a command is dangerous by extracting its effective target from a CLI's options (e.g. `git-safety.sh` reading `git -C <repo>`) is bypassable when it models the option with the wrong cardinality, order, or scope. Check three things against the REAL tool (run it — do not guess): (a) **cardinality/precedence** — a repeatable/cumulative option (`git -C a -C b` → git honors the LAST, cumulatively) modeled as 0-or-1, or a fixed order the tool doesn't require (`-C`-before-`-c`), lets a chained/reordered form slip the matcher entirely (→ ALLOW); (b) **matcher vs. extractor coupling** — relaxing the matcher regex WITHOUT teaching the extractor the same cumulative resolution is worse than no fix (matches, then resolves the wrong first/allowlisted target); (c) **subcommand scope boundary** — the option namespace changes at the verb, so a subcommand's own same-named flag (`git commit -C HEAD` reuse-message) must NOT be mined as the global option — extraction stops at the verb. (d) **multiple redirect vectors / independent targets** — a tool usually offers MORE than one way to redirect its target (git: `-C`, `--git-dir`/`GIT_DIR`, `--work-tree`/`GIT_WORK_TREE`, inline env prefix); enumerate the whole family, and if the tool resolves two related targets INDEPENDENTLY (git-dir for refs vs work-tree for files — a `--git-dir` redirect does NOT move the work-tree, so `git --git-dir=<safe>/.git reset --hard` from cwd=main still destroys main files), reconstruct and validate BOTH, or a redirect of the checked target masks a mutation of the unchecked one; when you close a redirect on target A, check whether the same class is now open on target B. The safety argument for a matcher change is that the new grammar is a strict SUPERSET of the old (can only add DENYs); a pre-fix red test reproducing the bypass on the live gate proves the hole was real. But an old-vs-new differential proves only NON-REGRESSION (no old-`DENY`→new-`ALLOW`) — it is BLIND to a new `ALLOW→ALLOW` bug, so it is NOT proof the gate is safe; the real proof is adversarial review plus positive tests that inputs empirically confirmed dangerous (run the tool, watch it mutate a marker file) now DENY. Prefer a crude-but-total check (validate all targets for all verbs) over a clever verb-aware one. Enumerate remaining residuals (a quoted redirect VALUE under an unquoted flag; cross-segment/exported/ambient env, structurally unseeable at the command-string layer) with the escape hatch named (Ref: `docs/solutions/logic-errors/command-gate-option-cardinality-and-verb-boundary-2026-07-20.md`; `.claude/hooks/git-safety.sh`)
23. **A gate's own LOCATOR is attack surface — review how the gate is found and invoked, not only what it decides** — a hook registered by a cwd-relative command (`bash .claude/hooks/git-safety.sh` in `.claude/settings.json`) resolves against a directory the agent changes for unrelated reasons, and the Bash tool's cwd **persists across calls**. Two defects follow: (a) **availability** — after any `cd` into a subdirectory the script is not found, and because hook failures are **non-blocking** the gate is silently SKIPPED, which is indistinguishable from ALLOW (a guard that cannot be found does not fail — it stops existing); (b) **selection**, the worse one — the relative form executes whatever `.claude/hooks/<name>.sh` sits under the current directory, and the harness trusts that script's stdout to emit the allow/deny verdict, so any checkout/fixture/dependency shipping a `.claude/hooks/` tree supplies the verdict — a confused deputy. Demand an absolute `bash "$CLAUDE_PROJECT_DIR/.claude/hooks/<name>.sh"` (double-quoted; arguments OUTSIDE the quotes). **Reject a defaulting fallback (`"${CLAUDE_PROJECT_DIR:-.}/…"`) on a gate's locator** — it looks defensive but silently restores the exact cwd-relative bug if the variable is ever unset, whereas the bare form fails loudly and TOTALLY at session start; on a gate the failure modes are not symmetric, and a quiet partial disarm outlives a loud total break (same asymmetry as rule 22's crude-but-total preference). Inside an already-located hook body the calculus flips: use `${BASH_SOURCE[0]}`-relative resolution for anything the hook **loads or executes** (`. "$ROOT/scripts/lib/…"`, `bash "$ROOT/scripts/…"`, `npx` binary/flat-config discovery — config files are executable code, so a cwd inside a package dir runs that package's config), but do NOT mechanically sweep repo-**state** queries (`git rev-parse HEAD`/`--show-toplevel`) to a script-derived root — for those, process cwd is often the semantically correct checkout (whose HEAD/drift matters), so decide per hook. Verification note: a live "cd to a subdirectory, confirm the guard fires" check is NOT possible in the session making the change — the harness reads registrations from `projectRoot` at session start (Ref: `docs/solutions/logic-errors/relative-locator-silently-disarms-non-blocking-hook-2026-07-25.md`; `.claude/hooks/test-settings-hook-paths.sh`)
24. **A deny-gate flag-presence check misses a standard spelling of the exact flag it names, or reads quote-blanked text where quoting is irrelevant** — a check gating on flag X being present in an ALREADY-CONFIRMED-real command clause (distinct from item 22's "is this command even invoked" question) commonly misses: the `--flag=value` spelling of a boolean flag, a glued short-form (`-XPOST` vs `-X POST`), and — the subtler one — a QUOTED instance of the flag (`gh pr merge 42 "--admin"` passes byte-identical argv to `gh` as the unquoted form; quotes only affect shell word-splitting, not what the program receives). The last one is a DIRECTIONAL misuse of this repo's `cmd_bare()` quote-blanking primitive: blanking quotes is correct for "is the command genuinely invoked, or merely MENTIONED in a quoted string" (prevents `git commit -m "never run eas update"` from false-positive-denying) but WRONG for "does this confirmed invocation carry flag X" — for that question, read the RAW command text with a widened non-alnum boundary class (`(^|[^-A-Za-z0-9])--flag([^-A-Za-z0-9]|$)`), not the quote-blanked form. A comment on the check reading "this can only ever ADD a deny, never grant a bypass" is a claim about polarity (presence→deny), not proof of detection completeness — verify by testing every standard spelling explicitly, not by trusting the polarity argument (Ref: `docs/solutions/logic-errors/deny-gate-flag-presence-check-needs-raw-text-and-every-spelling-2026-08-16.md`; `.claude/hooks/guard-outward-cli.sh`)
25. **A hand-spelled flag→value separator in a NEW detector re-derives an already-fixed redirect/substitution bypass** — when composing a new command-detection check (a `-X`/`--method`-style flag followed by its value) by reusing a shared library's position-anchor primitives (`_CMD_POS_PREFIX`, `cmd_words_deep`, etc.) but hand-spelling the separator BETWEEN the flag and its value as a bare character class (`([[:space:]]|=)*`), the check misses a shell redirect sitting in that slot (`-X 2>&1 PUT` genuinely invokes `-X PUT` — argv-confirmed under both bash and zsh — but the bare separator class doesn't absorb `2>&1`, so the value never matches adjacent to the flag) and a live command/process substitution supplying the value (`-X "$(echo PUT)"` renders as `cmd_words`'s placeholder text, never the literal method word, so a literal-value match can never see it). This is the SAME bug class item 22 names generically, but concretely: before writing ANY new flag→value check, grep the codebase (`guard-outward-cli.sh` is the usual owner) for an EXISTING check on the same or a similar flag shape — if one exists, it has almost certainly already been hardened (`_OUT_SEP`/`_CMD_REDIR`-absorbing separator, plus a separate "unreadable value co-occurring with $/backtick → deny" arm) and that hardened composition must be reused verbatim, not re-derived shorter. Verify by construction: build the exact adversarial command, confirm real argv with an argv-dumping stub under BOTH bash and zsh, then run it through the actual (not copied) detector before declaring either bug present or fixed (Ref: `docs/solutions/conventions/compose-precise-detector-from-shared-primitives-without-widening-extractor-2026-09-14.md`; `.claude/hooks/merge-review-guard.sh`; `.claude/hooks/guard-outward-cli.sh` "THE FLAG->VALUE SEPARATOR TAKES THE ABSORBER", 2026-09-07)
26. **A hook's fork-free self-location idiom silently fails open on a bare-filename invocation** — extending item 23's theme (a gate's own locator is attack surface) one layer INSIDE the hook, not at its registration: when converting `HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"` to a fork-free form for hot-path performance, a bare `HERE="${BASH_SOURCE[0]%/*}"` (parameter-expansion suffix removal) is NOT equivalent to `dirname`: on a slash-free `${BASH_SOURCE[0]}` (a bare-filename invocation, e.g. `cd .claude/hooks && bash x.sh`) it returns the string UNCHANGED instead of ".", so the subsequent `. "$HERE/lib/…"` silently fails to find a directory that never existed. Demand the guarded form — `case "${BASH_SOURCE[0]}" in */*) HERE="${BASH_SOURCE[0]%/*}" ;; *) HERE=. ;; esac` — on ANY diff replacing this idiom, and verify by executing a bare-filename invocation (`bash -x`, check the trace) rather than by reading the expression; every existing test and every real `.claude/settings.json` registration invokes hooks by a slash-qualified path, so nothing catches a regression here without a dedicated bare-invocation probe. Also check that the surrounding code treats a broken HERE (whatever the cause) as equivalent to "lib unsourceable" and routes to the hook's EXISTING fail-closed/fail-silent handling — never a new, unreviewed blind exit (Ref: `docs/solutions/logic-errors/naive-fork-free-here-silently-fails-open-on-bare-invocation-2026-09-02.md`)
27. **A diff that NARROWS, anchors, or "makes precise" a deny-shaped predicate is a regression candidate until base and head are run on the same GENERATED corpus in BOTH directions** — an enumerated character class, a fixed segment count or a closed trailing set inside a deny gate is an allowlist of shapes, and every shape the author did not list is an ALLOW. The narrower pattern always reads better (more specific, better commented, fixes a real over-denial), and green CI plus reconciled pins say nothing about it; #995's endpoint check failed open FIVE ways across four rounds before being restored to the broad substring test. Verify: extract the base guard with its `lib/` into a matching layout, generate the corpus from its dimensions (quoting is ALWAYS one when the predicate reads a rendering), report main-DENY/head-ALLOW and main-ALLOW/head-DENY separately, and include a mutation control that reproduces a non-zero fail-open count. Reject any "exhaustive" argument that cites a grammar (RFC, CLI help) instead of the exact string the predicate greps — `cmd_words_deep` replaces a quoted `#` with a placeholder, so a proof about URIs said nothing about the code. Past the second fail-open in one predicate, recommend restoring the broad predicate and documenting the over-denial rather than a further narrowing (the widening-direction mirror is code-reviewer's "swaps a matcher/guard for a BROADER shared one" rule; do not duplicate it here).
