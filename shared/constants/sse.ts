 ...
 * that enforces them and the client hooks that must stay ordered beneath them.
 *
 * `SSE_TIMEOUT_MS` is the server's own cap on every chat SSE connection —
 * coach, recipe and remix (`server/routes/chat.ts`). It must stay below
 * `client/hooks/useCoachStream.ts`'s `STREAM_INACTIVITY_MS`, which must stay
 * below its `XHR_TIMEOUT_MS`, and below `client/hooks/useChat.ts`'s
 * `CHAT_XHR_TIMEOUT_MS` (legacy coach + recipe chat) — on a
