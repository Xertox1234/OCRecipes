...
// Note: IDOR-protection and saved-item storage coverage lives in
// server/storage/__tests__/nutrition.test.ts, which exercises the real
// getSavedItems/getSavedItemCount/createSavedItem/deleteSavedItem functions
// against a Postgres test-transaction fixture (including the cross-user IDOR
// case). The previous inline `vi.fn()` blocks here asserted only their own
// stubs' return values and exercised zero production code, so they were removed.
