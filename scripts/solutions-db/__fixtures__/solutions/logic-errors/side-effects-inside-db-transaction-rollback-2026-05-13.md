...
symptoms:
  [
    Search index missing entries that still exist in the database,
    Stale entries linger until server restart re-runs `initSearchIndex`,
    Bug only surfaces when a transaction rolls back after a side effect already fired,
  ]
