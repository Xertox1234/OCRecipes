...
symptoms:
  [
    A parameter destructured alongside server-generated values is actually user-controlled,
    Variable name like `question` does not look like 'user message' so it bypasses sanitization conventions,
    Prompt template interpolates raw user input,
  ]
