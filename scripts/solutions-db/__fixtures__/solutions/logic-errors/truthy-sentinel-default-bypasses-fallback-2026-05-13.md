...
symptoms:
  [
    "`||` fallback never runs because the database default value is a non-empty string",
    Uncategorized rows keep their sentinel value instead of being re-categorized,
    Existing rows with default placeholders skip re-processing logic,
  ]
