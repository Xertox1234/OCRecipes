...
symptoms: [Every API request stalls 2–10ms reading the auth token from AsyncStorage, 'Rapid requests (e.g., infinite scroll, batch fetch) stutter on slower devices', Profiling shows AsyncStorage.getItem on the hot path of every request]
