...
symptoms: ['Route reads, writes, then reads again — only difference is the row just inserted', Double the DB round-trips for every write that needs an updated list, Insert returns the new row but the code re-queries instead of using it]
