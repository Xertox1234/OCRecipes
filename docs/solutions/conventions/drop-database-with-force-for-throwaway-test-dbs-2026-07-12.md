...
**Postgres floor bumped to 13+ (2026-07-13):** `WITH (FORCE)` requires PG13+, but this
codebase's documented minimum dev Postgres was 12+ (`docs/DEV_SETUP.md`, `docs/ARCHITECTURE.md`,
`README.md`) — one version below. On a PG12 install, `WITH (FORCE)` would be an
unrecognized-option error, silently swallowed by the same `2>&1` that hid the original bug,
reproducing this exact leak with zero signal that the fix didn't apply. Rather than add
version-detection/fallback logic to every cleanup trap, the documented floor was bumped to
13+ so this can't happen. If you're on an older local Postgres install below the documented
floor, upgrade, or use the `pg_terminate_backend` alternative under Exceptions below, which
