...
A new test file (`server/services/__tests__/barcode-lookup.test.ts`) mocked the `db` module's `insert(...).values(...)` chain with only `onConflictDoUpdate`. The code under test (`lookupBarcode`) actually calls `storage.insertBarcodeNutritionIfAbsent`, which chains `.onConflictDoNothing()` — a different method, absent from the mock. Every test in the file still passed: the missing method causes a real `TypeError` at runtime, but that call is a fire-and-forget background write (`storage.insertBarcodeNutritionIfAbsent(...).catch((err) => log.error(...))` in `barcode-lookup.ts`), so the thrown error is caught, logged, and never propagates to the test.
