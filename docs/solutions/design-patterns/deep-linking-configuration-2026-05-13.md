...
A deep-link target's not-found view also needs its **own exit**. `linking.ts` declares no intermediate screens for `chat/:conversationId`, so `getStateFromPath("chat/abc", linking.config)` builds a Coach stack holding only `Chat`: `canGoBack()` is false and the header shows no back button. A text-only not-found view is then a dead end. `NotebookEntryScreen` gets away with less because it is a `fullScreenModal` (always dismissible) and still renders a `goBack` `Pressable`. Render a labelled button that does `navigation.canGoBack() ? navigation.goBack() : navigation.navigate("ChatList")`. Do not escape via `setParams({ conversationId: undefined })`: the key stays present, so the malformed-id check stays true.
