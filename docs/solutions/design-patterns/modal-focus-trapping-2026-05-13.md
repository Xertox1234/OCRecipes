...
**Inline (non-portal) child Views within the same screen:** Having `accessibilityViewIsModal` on both a root container AND a nested child overlay simultaneously is safe and correct — they govern different sibling-sets at different tree levels (the root's prop hides navigator-behind siblings, the child overlay's prop hides camera/content siblings). The mutex-one-at-a-time rule only applies to portal-rendered modals (e.g. BottomSheetModal) placed as siblings OUTSIDE the root container. Do not add a mutex toggle for inline overlays — it violates the fullScreenModal always-on rule in `docs/rules/accessibility.md` and was confirmed CRITICAL by kimi-review in ScanScreen 2026-06-03.
