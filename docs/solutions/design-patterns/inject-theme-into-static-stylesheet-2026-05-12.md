...
symptoms: [A single colour in an otherwise-static StyleSheet needs to follow the theme, Component already uses useTheme() but its StyleSheet.create block can't read the hook, Tempted to restructure an entire stylesheet into a theme-function pattern for one colour]
