...
- Reset the display-scoped gate in the gesture's `.onBegin`, not `.onStart` or `.onEnd`. Resetting at the start means the very first `.onUpdate` frame of the new gesture is guaranteed to look like a transition. It must be `.onBegin` because `.onFinalize` fires on every terminal state (END/FAILED/CANCELLED) whether or not the gesture activated, and `.onStart` only fires on activation. On Android a single-finger tap drives the pinch recognizer BEGAN→FAILED (`PinchGestureHandler.kt`), so with an `.onStart`-only reset a gate left over from the previous pinch made every tap re-arm the label hide (found in review of #1081). **Also clear the gate inside `.onFinalize` once it has been used.** `onBegin` alone is not enough: on iOS a one-finger tap takes `UIPinchGestureRecognizer` from `.possible` straight to `.failed`, and RNGH fires `onFinalize` for any transition into FAILED but `onBegin` only for UNDETERMINED→BEGAN (`eventReceiver.ts`). So a failed iOS attempt gets `onFinalize` with no `onBegin` (found in the next review round). Clearing at the point of use doesn't depend on the platform's event order.
