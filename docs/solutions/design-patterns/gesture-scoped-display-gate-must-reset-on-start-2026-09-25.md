...
# A per-gesture "last shown" bridge gate must reset at gesture begin; a "last applied" native-state gate must not
...
- **Clear the display-scoped gate at its point of use, inside `.onFinalize`, right after arming the hide.** That is the reset the fix depends on: every gesture that wrote the gate reached `.onUpdate`, so it was ACTIVE and ends in END/FAILED/CANCELLED, all of which reach `.onFinalize` (RNGH `eventReceiver.ts`'s shared terminal branch). The next gesture's first `.onUpdate` then always looks like a transition, and an attempt that never activates finds the gate already clean, whatever order the platform delivers events in. Also reset it in `.onBegin` as cheap defence in depth (covers a terminal event that never arrives, e.g. the detector unmounting mid-gesture). Don't rely on `.onStart`: it fires only on activation, while `.onFinalize` also fires for attempts that never activate; on Android a single-finger tap drives the pinch recognizer BEGAN→FAILED (`PinchGestureHandler.kt`), which is how an `.onStart`-only reset made every tap after a pinch re-arm the label hide (review of #1081). Whether iOS delivers anything at all for a one-finger tap on the pinch recognizer (`.possible`→`.failed`) was read from source and is disputed between reviewers; it is not device-verified, and the point-of-use clear makes it irrelevant.
