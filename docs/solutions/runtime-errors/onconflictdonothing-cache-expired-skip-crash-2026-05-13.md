...
symptoms:
  - "Cache refresh appears to succeed but the TTL'd row never changes"
  - "Non-null assertion (`!`) crashes the request on cache refresh"
  - "Cache table holds an expired row that no new write can overwrite"
created: 2026-04-28
severity: high
---
