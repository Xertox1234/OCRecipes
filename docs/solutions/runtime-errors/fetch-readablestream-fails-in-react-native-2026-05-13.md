...
symptoms: ['`response.body` is `null` for `text/event-stream` responses in React Native', '`isStreaming` stays `true` but `streamingContent` stays empty', SSE works in a browser but silently fails in React Native]
