...
**Enforced at commit time since 2026-08-11.** `scripts/check-solution-frontmatter.js` fails any staged doc whose `tags` match **no** routed domain at all — the universally-unreachable case, where the doc is in no domain's candidate pool and `applies_to` cannot rescue it. It does not (and cannot cheaply) check the narrower "this glob's domain isn't in my tags" case above, so the worked example still needs your judgement. Note the two alternations the hook honours: `harness` also accepts `tooling`/`pg-lab`/`worktree(s)`/`agents`, and `ai-prompting` accepts any `ai-*` tag. **144 pre-existing docs violate this and are NOT exempt — there is no allowlist.** The check only fires on docs you *stage*, so it is a ratchet, not an amnesty: an untouched violator is never checked, but **editing one blocks the commit until you add a routed-domain tag**. That is intentional (the fix is one line in the file you already have open), but it has two consequences worth knowing before you start:

- The `/codify` near-dup rule prefers *extending* an existing doc over writing a new one. If the doc you extend is one of the 144, expect to fix its tags in the same commit.
- A bulk corpus pass (prune, dedup, re-tag) will hit this once per violator it touches. Each needs a judgement call about which domain applies, so budget for it rather than reaching for `git commit --no-verify`.
