...
symptoms: [TalkBack reads the same status text twice in rapid succession, Adding `accessibilityLiveRegion="polite"` to a view that already uses `announceForAccessibility` causes duplicate speech, kimi-review flags double-announcement on selection-count chip]
