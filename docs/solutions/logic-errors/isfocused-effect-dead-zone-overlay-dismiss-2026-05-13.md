...
symptoms:
  - "Scanner remains permanently blocked after dismissing an in-screen overlay"
  - "useEffect on `isFocused` never re-runs when overlay closes on the same focused screen"
  - "Reset/re-initialize logic relies on `isFocused` but the screen stayed focused"
created: 2026-05-02
severity: high
---
