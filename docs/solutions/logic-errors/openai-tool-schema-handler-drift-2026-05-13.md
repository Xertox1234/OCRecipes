...
symptoms: [Tool handler reads `args.X` but `X` is not declared in the OpenAI schema, Tool call silently defaults to `0` or `undefined` for the missing parameter, Schema declares a property the handler never reads]
