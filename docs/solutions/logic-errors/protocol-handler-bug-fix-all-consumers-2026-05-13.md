...
applies_to:
  [
    "client/components/CoachChat.tsx",
    "client/components/CoachOverlayContent.tsx",
  ]
symptoms:
  - "Two components implement the same wire protocol via copy-paste"
  - "Fixing the bug in one leaves an identical bug in the other"
  - "`isStreaming` stuck `true` because server-sent error events are silently dropped"
created: 2026-04-12
severity: high
---
