...
symptoms:
  - "Unmounted-component warnings after a network error in a `useEffect`-launched mutation"
  - "`onSuccess` guards with a `cancelled` ref but `onError` does not"
  - "Mutation `onError` path mutates state on a screen that has already unmounted"
created: 2026-04-28
severity: medium
---
