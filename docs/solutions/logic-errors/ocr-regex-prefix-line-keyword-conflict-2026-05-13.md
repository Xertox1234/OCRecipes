...
symptoms:
  - "Parser captures 'from' as the calorie value on pre-2020 US labels"
  - "`parseFloat('from')` returns `NaN`, calorie count silently becomes `null`"
  - "First match in `String.match()` wins — wrong line ahead of the real one"
created: 2026-04-07
severity: high
---
