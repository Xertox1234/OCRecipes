...
that makes `isOnRootScreen` false does **not** unmount `ScanFAB`. An `unmount`-only cleanup
effect written to reset an orphaned `menuOpen: true` therefore never fired for whatever case
flips `isOnRootScreen`,
