...
symptoms: [Deleting a parent row fails with CHECK constraint violation, User account deletion blocked because the cascade chain hits a CHECK, CRUD tests pass; deletion-cascade only fails when parent rows exist]
