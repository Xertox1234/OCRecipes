...
- A new variant: after a blur/refocus round-trip (e.g. deep-link push then back press), a state-driven sheet that was left open may experience a spurious onSheetChange(-1) from gorhom while the app's own isOpen prop never changed. The isOpenRef gets stuck false with no recovery path, causing hardware-back presses to silently stop dismissing the still-visible sheet. This is a desync between the animation-derived ref and the authoritative application state, not a race condition during transitions.
...
**Update (2026-07-13) — spurious onSheetChange(-1) during blur/refocus round-trip, new failure mode for state-driven hosts**

A *distinct* failure mode, not a timing race, was discovered in the same `useSheetBackHandler` hook during testing on Android emulator. After a deep-link push blurred a screen with a state-driven sheet open, and then a hardware-back pop refocused it, further hardware-back presses stopped dismissing the still-open sheet — even though `isFocusedRef` correctly settled back to `true` (verified via a pre-existing unit test) and no competing/leaked JS `BackHandler` listener existed on that path (only one other JS `hardwareBackPress` listener exists app-wide: React Navigation's own, registered once at app boot — verified via a `todo-researcher` subagent).

**Root cause:** `isOpenRef` is written `true` only when the `isOpen` prop transitions to `true` (an effect keyed on `[isOpen]`), and written `false` only by `onSheetChange(-1)`. If a stray/spurious `onSheetChange(-1)` fires from gorhom while the state-driven `isOpen` boolean itself never changes (stays `true` through the whole blur/refocus round-trip — the host's own trigger state was never cleared), `isOpenRef` gets stuck `false` with **no recovery path**, since nothing ever re-triggers the `isOpen`-transition effect. The exact native trigger of the spurious `onSheetChange(-1)` during this specific round-trip was **not conclusively pinned** in this session (researched candidates: the pushed screen's `presentation:"modal"`+`gestureEnabled` config in `RootStackNavigator.tsx` interacting with RN 0.81.5's AndroidX `OnBackPressedDispatcher` migration for Android 16/API-36 predictive-back support — but this remains a hypothesis, not a confirmed mechanism). This is documented as an open question, not a solved mystery.

**Fix (defensive, not root-cause-eliminating):** added a new `stateIsOpenRef` that unconditionally mirrors the live `isOpen` prop in **both** directions (`true` and `false`) via its own `useEffect` (the same `useEffect`-mirror-for-async-callback pattern already used for `isFocusedRef` in the same file). The hardware-back handler now treats the sheet as open if **either** `isOpenRef.current` **or** `stateIsOpenRef.current === true` — i.e. it trusts the app's own authoritative 'should be open' state as a fallback whenever the animation-derived ref has desynced. This fix is **scoped to state-driven hosts only**: imperative hosts (no `isOpen` passed — `HomeScreen`, `RecipeBrowserScreen`, `RecipeEntryHubScreen`, `BeveragePickerSheet`, `ConfirmationModal`) have no authoritative fallback source and remain exposed to the same class of issue with no recovery path — this is a known, documented gap, not yet fixed. Verified the OR-logic is monotonic-safe (can only add 'open' cases, never remove them) so it provably cannot regress the existing close-animation grace period or the focus gate, and correctly no-ops for imperative hosts (`isOpen` stays `undefined` there, so `stateIsOpenRef.current === true` is always `false`).

Two new regression tests added to `client/hooks/__tests__/useSheetBackHandler.test.ts`: (1) the repro (spurious `onSheetChange(-1)` while `isOpen` stays `true` through a blur/refocus round-trip — asserts a further back press still dismisses), and (2) the symmetric negative case (`isOpen` flips `false` for a real close, confirmed by `onSheetChange(-1)` — asserts a post-refocus back press still falls through, proving the fix can't make a genuinely-closed sheet 'sticky').

See `todos/archive/P3-2026-07-12-sheetbackhandler-stale-listener-after-blur-refocus.md` for the source todo and the full investigation notes.
...
- For state-driven hosts, be aware that gorhom may fire a spurious `onSheetChange(-1)` during a blur/refocus round-trip even when the sheet remains open. The defensive fix uses an authoritative fallback ref (`stateIsOpenRef`) that unconditionally mirrors the `isOpen` prop. Ensure your hook implementation includes this fallback for state-driven hosts; imperative hosts remain vulnerable to this class of desync and should be monitored for similar symptoms.

## Related Files

- `client/hooks/useSheetBackHandler.ts` — the hook that surfaced this; `onSheetAnimate`/`onSheetChange` implement the fix above, now applied to BOTH usage modes (the `isOpen`-mirror effect only ever opens the ref; closing is confirmed exclusively by `onSheetChange(-1)`). Also includes the `stateIsOpenRef` fallback for state-driven hosts (added 2026-07-13).
- `client/hooks/__tests__/useSheetBackHandler.test.ts` — test coverage for the animation-start vs. animation-complete ordering, the state-driven closing-grace-period case, the `useIsFocused()` focus-scoping gate, and the 2026-07-13 regression tests for spurious `onSheetChange(-1)` during blur/refocus.
...
- `todos/archive/P3-2026-07-12-sheetbackhandler-stale-listener-after-blur-refocus.md` — source todo documenting the 2026-07-13 failure mode, investigation notes, and the open question regarding the native trigger.
