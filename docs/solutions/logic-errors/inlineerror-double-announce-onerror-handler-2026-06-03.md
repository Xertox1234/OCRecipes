...
symptoms: [VoiceOver reads the error message twice in rapid succession after a mutation fails, TalkBack reads the error message twice after a failed save, 'Two different error strings are spoken — one from the onError handler, one from InlineError']
