...
module: client
severity: high
tags: [client-state, auth, error-handling, irreversible-operations, async-storage]
symptoms: [User taps Retry after an account-delete error and hits 401 because the account is gone, Generic 'try again' error surfaces in UI after a successful server mutation, Local storage rejection propagates through a hook after server already confirmed deletion]
