...
- Classifier / safety regexes that consume across newlines must bound every `.*` and `[\s\S]*` gap (e.g. `[\s\S]{0,500}`) — never leave an unbounded quantifier, even on a new arm. Bound for consistency with the file's existing bounded patterns, not just where the new arm is independently exploitable: an unbounded arm next to bounded siblings re-trips every future ReDoS review and silently widens the backtracking surface as the alternation grows.
