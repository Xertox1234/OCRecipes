...
**Category:** Gotcha — Pre-commit hooks / Generated artifacts

Generated files committed to the repo for CI byte-equality checking (e.g., `.github/copilot-instructions.md` produced by `scripts/build-copilot-instructions.ts`) get silently reformatted by the `lint-staged` pre-commit hook unless they are explicitly excluded from Prettier. The next time the generator runs, its output differs from the committed bytes — and the `--check` CI step fails on a file the developer didn't touch.

**Symptom observed in PR #149:** Committed `.github/copilot-instructions.md` matched `npm run build:copilot-instructions:check` at the moment of `git commit`, but Prettier ran inside the lint-staged hook between staging and the commit landing. Prettier:

- Padded markdown table columns from `| --- | --- |` to `| ------- | ------- |` for alignment
- Changed italic delimiters from `*asterisk*` to `_underscore_`

The committed bytes diverged from the script output by ~hundreds of cosmetic characters. The `--check` step would have failed CI on the first run after merge.

**Root cause:** Prettier's lint-staged hook runs after `git add` but before the commit is finalized, so even if `npm run build:foo:check` passes when you manually verify pre-commit, the bytes that actually land in HEAD have been touched by the formatter.

**Fix:** Add the generated file to `.prettierignore` with a comment explaining why:
...
After ignoring, regenerate (`npm run build:copilot-instructions`) and commit. The bytes now stay stable across commit cycles.

**Generalization:** Any tracked, generated artifact whose CI gate is byte-equality with the generator's output MUST be in every formatter's ignore list — Prettier, ESLint `--fix`, dprint, biome, gofmt, etc. The pattern applies equally to generated TypeScript stubs, generated JSON schemas, generated Markdown docs, and generated workflow YAML.

**Detection:** The byte-equality `--check` mode catches this on the FIRST CI run after the artifact lands. A holistic branch-level pre-PR review can catch it locally — but you have to run `npm run build:foo:check` AFTER the commit lands (not before staging), since the formatter mutation happens inside the commit boundary.
...
36. **Bash `case` is exclusive — use independent `if` blocks for additive multi-domain matching:** A `case "$VAR" in pattern) ...; ;; esac` block matches the first arm only and stops. When a single input needs to accumulate results from multiple independent rules (e.g., an AI service file needing both `ai-prompting` and `architecture` domains, a test file inside a route directory needing both `api` and `testing`), a single `case` silently drops everything after the first match. Replace with independent `if [[ ... ]]` blocks so all matching branches execute. Caught during spec compliance review of `.claude/hooks/inject-patterns.sh` — AI service files were missing the `architecture` domain and `server/routes/__tests__/` files were missing `testing`.
